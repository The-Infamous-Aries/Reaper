...
class SnipeGuide(commands.Cog):
    ...
    async def snipe_guide(self, ctx: commands.Context):
        ...
        setup_text = f"""
Now i would like to start of by saying that you do **NOT** have to raid using the <@946351598223888414> bot! .. You can spend several minutes trolling nations and alliances in-game and hope you find someone unallied to anyone who will counter you who also has money 🥱 or you can raid like the **Non-Countered Raider** I am gonna turn you into! .. Now assuming your still reading this and not playing with pets like a nerd 🤓, I would like to **GREATLY** emphasize that it is **CRUCIAL** you are using a time app/site and preferable on 2 screens unless your good at timing (not me I got the rhythm of a limping T-Rex 🦖) or you will be missing out on the beige targets with **LOTS** of money 💸😭 .. Now follow The Reapers's 10 steps of raiding and you will be set; 
...
Now that you know how to raid, get out there and show me what you got! .. Always remember; *Take what you can, Give nothing back!* 🏴‍☠️"""
            ...
            if mode_key == "margin":
                ...
                if best_buy_price is not None and best_sell_price is not None:
                    ...
                    if best_buy_price is not None and best_buy_price != 0:
                        ...
                        display_price = f"Buy: ${best_buy_price:,} | Sell: ${best_sell_price:,} | Margin: ${margin:,} ({percent_margin:.2f}%)"
                    else:
                        display_price = f"Buy: ${best_buy_price:,} | Sell: ${best_sell_price:,} | Margin: ${margin:,} (N/A%)"
                else:
                ...
                display_price = f"${price:,}" if price else no_offer_text

            emoji = emoji_map.get(res)
            if prefer_emoji and emoji:
                line = f"{emoji} {fmt_name(res)}: {display_price}"
            else:
                line = f"{fmt_name(res)}: {display_price}"
...
Now that you know how to raid, get out there and show me what you got! .. Always remember; *Take what you can, Give nothing back!* 🏴‍☠️"""
    ...
    async def war_guide(self, ctx: commands.Context, category: Optional[str] = None):
        ...
        categories = {
            "ground_sup": {
                "title": f"Ground Supremacy 🪖",
                "content": f"""# Ground Supremacy 🪖
...
• This allows you to target both ground and air units simultaneously"""
            ...
            "air_sup": {
                "title": f"Air Supremacy ✈️", 
                "content": f"""# Air Supremacy ✈️
...
• Provides tactical flexibility to strike any enemy forces"""
            ...
            "naval_sup": {
                "title": f"Naval Blockade/Supremacy 🚢",
                "content": f"""# Naval Blockade/Supremacy 🚢
...
• VDS ALWAYS prevents 1 non-power plant, non-military improvement from being destroyed"""
            ...
            "fortification": {
                "title": f"Fortification 🛡️",
                "content": f"""# Fortification 🛡️
...
• **Rule:** Never peace alliance-sanctioned attacks until told to do so!"""
            ...
            "strategy": {
                "title": f"Key Strategy 📋",
                "content": f"""# Key Strategy 📋
...
async def setup(bot: commands.Bot):
    ...
    if alliance_cog:
        ...
        if query_instance and calc_instance:
            ...
            try:
                from Systems.PnW.IA.alliance import create_query_instance, AllianceCalculator
