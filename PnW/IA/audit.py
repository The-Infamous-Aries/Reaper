...
class AuditManager(commands.Cog):
    ...
    async def _fetch_flag_image(self, url: str) -> Optional[Image.Image]:
        ...
        try:
            ...
            async with aiohttp.ClientTimeout(total=10) as session:
    ...
    async def _get_combined_nations(self) -> List[Dict[str, Any]]:
        ...
        cy = await self._get_alliance_nations(int(self.db4d_id), force_refresh=True)
    ...
    async def _add_category_fields(self, embed: discord.Embed, category_name: str, emoji: str, items: List[Dict[str, Any]], with_days: bool = False, suffix_builder: Optional[Callable[[Dict[str, Any]], Optional[str]]] = None):
    ...
    async def _audit_mmr_sync(self, active_members: List[Dict[str, Any]], mmr_mode: str) -> discord.Embed:
        ...
        if total_offenders == 0:
            ...
            await self._add_category_fields(embed, "50%+ off", "🟥", bucket_50_plus, suffix_builder=suffix_builder)
            await self._add_category_fields(embed, "25–49% off", "🟧", bucket_25_49, suffix_builder=suffix_builder)
            await self._add_category_fields(embed, "10–24% off", "🟨", bucket_10_24, suffix_builder=suffix_builder)
            await self._add_category_fields(embed, "0–9% off", "🟩", bucket_0_9, suffix_builder=suffix_builder)
    ...
    @commands.hybrid_command(name="audit", description="Audit alliance issues for DB4D")  # type: ignore
    async def audit_command(self, ctx: commands.Context, view: Literal["resources", "inactives", "color", "mmr"], mmr_mode: Optional[Literal["basic", "max"]] = "basic"):
        ...
        try:
            ...
            if view == "mmr":
                try:
                    embed = await self._audit_mmr_sync(active_members, mmr_mode or "basic")
            ...
            if view_key == "resources":
                ...
                await self._add_category_fields(embed, "Wrong Color", "🎨", wrong_color)
            ...
            try:
                ...
                if channel_id and channel_id in self.treaties_message_map:
                    try:
                        ...
                        edited = False
                if not edited:
                    sent = await ctx.send(embed=embed, view=view, files=files if files else [])  # type: ignore
                    try:
                        if channel_id:
...
async def setup(bot: commands.Bot):
    ...
    if alliance_cog:
        ...
        if query_instance and calc_instance:
            ...
            try:
                bot.add_view(TreatiesRefreshView(audit, int(audit.db4d_id), timeout=None))
            except Exception:
                pass
