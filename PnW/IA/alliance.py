...
class FullMillView(discord.ui.View):
    ...
    def __init__(self, author_id: int, bot: commands.Bot, query_instance: PNWAPIQuery, calc_instance: AllianceCalculator, nations: List[Dict] = [], target_alliance_id: Optional[str] = None, target_alliance_name: Optional[str] = None):
        ...
        self.target_alliance_name = target_alliance_name
    ...
    async def generate_full_mill_embed(self, nations: List[Dict] = []) -> discord.Embed:
        ...
        try:
            ...
            if not current_nations:
                ...
                guild_alliance_id = HOME_ALLIANCE_ID
            ...
            embed = discord.Embed(
                title=f"{emoji_mod.mention('IA') or '🏭'} {self.target_alliance_name or 'DB4D'} Military Analysis",
...
class AllianceTotalsView(discord.ui.View):
    ...
    async def generate_alliance_totals_embed(self, nations: List[Dict] = []) -> discord.Embed:
        ...
        try:
            ...
            if not current_nations:
                # Get alliance data based on guild
                guild_nations = await self.query.get_alliance_nations(HOME_ALLIANCE_ID)
            ...
            is_db4d = str(self.target_alliance_id or HOME_ALLIANCE_ID) == '14635'
            ...
            if is_db4d:
                embed = discord.Embed(
                    title=f"{emoji_mod.mention('credit') or '💰'} {self.target_alliance_name or 'DB4D'} Resources",
            ...
            embed = discord.Embed(
                title=f"{emoji_mod.mention('Stat') or '📊'} {self.target_alliance_name or 'Alliance'} Totals",
    ...
    async def full_mill_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if not self.current_nations:
                # Fetch alliance data based on guild
                nations = await self.query.get_alliance_nations(HOME_ALLIANCE_ID)
                if not nations:
                    embed = discord.Embed(
                        title=f"{emoji_mod.mention('Deny')} No Alliance Data",
                        description="Failed to retrieve alliance data.",
...
class ImprovementsView(discord.ui.View):
    ...
    def __init__(self, author_id: int, bot: commands.Bot, query_instance, calc_instance, alliance_cog, nations: Optional[List[Dict]] = None, target_alliance_id: Optional[str] = None, target_alliance_name: Optional[str] = None):
        super().__init__(timeout=300)  # 5 minute timeout
        self.author_id = author_id
        self.bot = bot
        self.query = query_instance
    ...
    async def generate_improvements_embed(self, nations: List[Dict] = []) -> discord.Embed:
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"{emoji_mod.mention('IA') or '🏗️'} {self.target_alliance_name or 'DB4D'} Improvements Breakdown",
...
class ProjectTotalsView(discord.ui.View):
    ...
    def __init__(self, author_id: int, bot: commands.Bot, query_instance, calc_instance, nations: Optional[List[Dict]] = None, target_alliance_id: Optional[str] = None, target_alliance_name: Optional[str] = None):
    ...
    async def generate_project_totals_embed(self, nations: List[Dict] = []) -> discord.Embed:
        try:
            ...
            embed = discord.Embed(
                title=f"🏛️ {self.target_alliance_name or 'DB4D'} Project Totals",
...
class AllianceManager(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, query_instance: PNWAPIQuery, calc_instance: AllianceCalculator):
        self.bot = bot
    ...
    async def generate_alliance_totals_embed(self, nations: List[Dict[str, Any]], target_alliance_name: Optional[str] = None) -> discord.Embed:
        ...
        try:
            ...
            view = AllianceTotalsView(self.bot.user.id if self.bot.user else 0, self.bot, self.query_system, self.calc_system, nations, target_alliance_id=str(self.db4d_alliance_id), target_alliance_name=target_alliance_name)
...
async def setup(bot: commands.Bot):
    ...
    try:
        ...
        query_instance = create_query_instance(api_key=PANDW_API_KEY, logger=logger)
