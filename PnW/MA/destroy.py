...
from Systems.Functions.emoji import SPY_EMOJI, SOLDIER_EMOJI, TANK_EMOJI, JET_EMOJI, SHIP_EMOJI
from config import PANDW_API_KEY, HOME_ALLIANCE_ID
from Systems.Functions.user_data_manager import UserDataManager

from Systems.PnW.Util.calc import AllianceCalculator, BuildingRatios
...
class DestroyCog(commands.Cog):
    ...
    def _safe_get(self, data: Dict[str, Any], key: str, default: Any = None, expected_type: Optional[type] = None) -> Any:
        """
    ...
    async def load_blitz_parties(self, user_id: int) -> List[Dict[str, Any]]:
        """
        Load the most recent blitz party data for a specific user.
        
        Args:
            user_id: The ID of the user whose blitz parties to load.
        ...
        try:
            ...
            return user_data.get("blitz_parties", [])
        except Exception as e:
    ...
    def calculate_combat_score(self, nation: Dict[str, Any]) -> float:
        ...
        try:
            if not self.calculator:
                self._log_error("AllianceCalculator not available for combat score calculation", context="calculate_combat_score")
                return 0.0
    ...
    def has_project(self, nation: Dict[str, Any], project_name: str) -> bool:
        ...
        try:
            if not self.calculator:
                self._log_error("AllianceCalculator not available for project checking", context="has_project")
                return False
    ...
    def create_optimal_attackers_view(self, interaction: discord.Interaction, target_nation: Dict[str, Any], optimal_attackers: Dict[str, Any], show_warchest: bool = False) -> Optional['OptimalAttackersView']:
    ...
    async def destroy(
        ...
        try:
            ...
            try:
                ...
                try:
                    assert sent_target is not None
            ...
            if view.attacker_pages:
                for idx, page in enumerate(view.attacker_pages):
                    ...
                    try:
                        ...
                        try:
                            assert sent_attacker is not None
...
class OptimalAttackersView:
    ...
    def create_target_message(self) -> str:
        ...
        try:
            ...
            building_ratios: BuildingRatios
            try:
                if self.cog.calculator:
