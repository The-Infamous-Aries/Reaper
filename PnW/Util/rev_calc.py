...
def calc_infra_value(starting_amount: float, ending_amount: float) -> float:
    """Calculates the cost of infrastructure based on a chunking mechanism similar to the game's PHP source."""
    start = round(float(starting_amount), 2)
    end = round(float(ending_amount), 2)
    
    diff = end - start
    if diff <= 0:
        return 150.0 * diff if diff < 0 else 0.0

    total_cost = 0.0
    current_amount = start
    
    while current_amount < end:
        remaining_diff = end - current_amount
        chunk_size = 0.0
        
        if remaining_diff > 100:
            # This logic mimics the recursive PHP function's chunking strategy
            if remaining_diff % 100 != 0:
                chunk_size = remaining_diff % 100
            else:  # remaining_diff is a multiple of 100
                chunk_size = 100.0
        else:  # remaining_diff <= 100
            chunk_size = remaining_diff
        
        price_at_chunk_start = round(infra_price(current_amount), 2)
        total_cost += price_at_chunk_start * chunk_size
        current_amount += chunk_size
        # Round to prevent floating point inaccuracies from accumulating
        current_amount = round(current_amount, 2)
            
    return total_cost

def infra_purchase_cost(current_infra: float, infra_to_buy: float, projects: Optional[set] = None, total_cities: int = 0, domestic_policy: Optional[str] = None) -> float:
    base = calc_infra_value(current_infra, current_infra + infra_to_buy)
    mult = 1.0
    if check_project_requirements("Advanced Engineering Corps", projects or set(), total_cities):
        mult *= 0.95
...
def land_purchase_cost(current_land: float, land_to_buy: float, projects: Optional[set] = None, total_cities: int = 0, domestic_policy: Optional[str] = None) -> float:
...
def city_purchase_cost(city_to_buy: int, top_20_average: float) -> float:
...
def project_build_cost(project_name: str, domestic_policy: Optional[str] = None) -> Dict[str, int]:
...
def calculate_full_revenue(
    ...
    domestic_policy: Optional[str] = None,  # Domestic policy selection
) -> Dict[str, Any]:
    ...
    tax_rate: float = alliance_data.get("tax_rate", 0) / 100.0
    ...
    if domestic_policy:
        ...
        if domestic_policy == "Open Markets":
            effects_applied_list.append("1% gross income bonus")
        elif domestic_policy == "Imperialism":
            effects_applied_list.append("5% military upkeep reduction")
        elif domestic_policy == "Urbanization":
            effects_applied_list.append("5% infrastructure cost reduction")
        elif domestic_policy == "Rapid Expansion":
            effects_applied_list.append("5% land cost reduction")
        elif domestic_policy == "Manifest Destiny":
            effects_applied_list.append("5% city cost reduction")
        elif domestic_policy == "Technological Advancement":
            effects_applied_list.append("5% project cost reduction (money only)")
