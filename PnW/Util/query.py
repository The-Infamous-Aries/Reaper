...
class PNWAPIQuery:
    ...
    def __init__(self, api_key: Optional[str] = None, logger: Optional[logging.Logger] = None):
        ...
        if api_key is not None:
            ...
            if CONFIG_PANDW_API_KEY is not None:
                self.api_key = CONFIG_PANDW_API_KEY
            else:
                raise ValueError("P&W API key not configured. Please set PANDW_API_KEY in your .env file or pass it to the constructor.")
        ...
        self.local_tz: Optional[ZoneInfo]
        try:
            self.local_tz = ZoneInfo(self.local_tz_name)
        except Exception:
            self.local_tz = None
        self.utc_tz: Optional[ZoneInfo]
        try:
        ...
        self._trade_cache: Optional[List[Dict[str, Any]]] = None
        self._trade_cache_expiry: float = 0.0
        self._trade_cache_ttl_seconds = 5
        ...
        self._processing_alliances: set[str] = set()
        self._processing_projects: set[str] = set()
        self._processing_improvements: set[str] = set()
    ...
    async def search_alliances(self, text: str, max_results: int = 25) -> Optional[List[Dict[str, Any]]]:
        try:
            ...
            try:
                url = "https://politicsandwar.com/api/alliances/?key=" + self.api_key + "&search=" + quote(str(text))
    ...
    async def resolve_alliance(self, identifier: Union[str, int]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            aid: Optional[Union[int, str]] = None
            try:
                m = None
                try:
                    import re as _re
            ...
            # Convert aid to int if it's a string digit
            ...
            try:
                ...
                static_map = {
                    'db4d': HOME_ALLIANCE_ID,
                    'death before dishonor': HOME_ALLIANCE_ID,
                }
    ...
    async def get_trade_resource_values(self, resources: Optional[List[str]] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if self._trade_cache and now < self._trade_cache_expiry:
                ...
                block = (data.get("data") or {}).get("top_trade_info") or {}
                resource_list = block.get("resources") or []
                if not resource_list:
                    self.logger.warning("get_trade_resource_values: top_trade_info returned no data")
                    return []
                # Update cache
                self._trade_cache = resource_list
                self._trade_cache_expiry = time.monotonic() + self._trade_cache_ttl_seconds

            full_resource_data = {item['resource']: item for item in self._trade_cache}

            if resources:
                requested = {str(r).upper() for r in resources}
                keys = [k for k in full_resource_data.keys() if k in requested]
            else:
                keys = list(full_resource_data.keys())

            result: List[Dict[str, Any]] = []
            for k in keys:
                resource_data = full_resource_data.get(k, {})
                average_price = float(resource_data.get("average_price", 0) or 0)
                best_buy_offer = resource_data.get("best_buy_offer", {"price": 0, "quantity": 0})
                best_sell_offer = resource_data.get("best_sell_offer", {"price": 0, "quantity": 0})

                result.append({
                    "resource": k,
                    "average_price": average_price,
                    "best_buy_offer": {
                        "price": float(best_buy_offer.get("price", 0) or 0),
                        "quantity": int(best_buy_offer.get("quantity", 0) or 0),
                    },
                    "best_sell_offer": {
                        "price": float(best_sell_offer.get("price", 0) or 0),
                        "quantity": int(best_sell_offer.get("quantity", 0) or 0),
                    },
                })
            return result
        except Exception as e:
        ...
        try:
            ...
            for mode in modes:
                ...
                while len(finished) < len(ids):
                    ...
                    query = "query {\n" + "\n".join(blocks) + "\n}"
                    self.logger.info(f"GraphQL Query for alliances {batch_ids}: {query}")
                    ...
                    try:
                        ...
                        self.logger.info(f"Raw API response for alliances {batch_ids}: {data}")
                    except Exception:
    ...
    async def get_improvements_data(self, alliance_id: str, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            improvements_counts: dict[str, int] = {}
            for nation in nations:
