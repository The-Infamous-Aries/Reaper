...
class AstrologyView(discord.ui.View):
    ...
    @discord.ui.button(label="Primal", style=discord.ButtonStyle.secondary, emoji="🌀")
...
class AstrologyCog(commands.Cog):
    ...
    async def _fetch_api_ninjas_bundle(self, sign: str) -> (Optional[str], Optional[Dict[str, Any]]):
        """Fetch daily horoscope from API Ninjas."""
        if not config.HORSCOPE_API:
             return None, None
             
        sign_slug = str(sign).strip().lower()
        url = f"https://api.api-ninjas.com/v1/horoscope?sign={sign_slug}"
        try:
            if aiohttp is None:
                return None, None
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers={'X-Api-Key': config.HORSCOPE_API}, timeout=10) as resp:
                    if resp.status != 200:
                        return None, None
                    data = await resp.json()
                    text = str(data.get("horoscope") or "").strip() or None
    ...
    async def _fetch_daily_bundle(self, sign: str) -> (Optional[str], Optional[Dict[str, Any]]):
        """Unified fetch: try API Ninjas first, then fall back to Aztro/Ohmanda."""
        text, stats = await self._fetch_api_ninjas_bundle(sign)
        if text:
            return text, stats
    ...
    def _build_horoscope_embed(self, sign: str, text: str, stats: Optional[Dict[str, Any]], sign_data: Optional[Dict[str, Any]] = None) -> discord.Embed:
        ...
        embed.set_footer(text="Powered by free horoscope APIs • Allspark Astrology")
        return embed
    ...
    async def send_all_horoscopes_to_channel(self, channel_id: int):
        ...
        for sign in self._WESTERN_SIGNS:
            try:
                ...
                sign_data = await self._find_sign_data(sign)
                fallback_text = text or "Daily horoscope is unavailable right now."
                embed = self._build_horoscope_embed(sign, fallback_text, stats, sign_data)
    ...
    def _build_unified_embed(self, western_data: Dict[str, Any], chinese_data: Dict[str, Any], 
                           ...
                           western_sign: str, chinese_animal: str) -> discord.Embed:
        # This is now a fallback or used for the first page
        return self._build_western_embed(western_data, user_birthday, western_sign)

    @commands.hybrid_command(name="horoscope", description="Get the daily horoscope for a sign")
    @app_commands.describe(sign="The zodiac sign")
    ...
    async def horoscope(self, ctx: commands.Context, sign: str):
        ...
        text, stats = await self._fetch_daily_bundle(match)
        sign_data = await self._find_sign_data(match)
        
        if not text:
            msg = f"❌ Could not fetch horoscope for {match}."
            if ctx.interaction:
                await ctx.interaction.followup.send(msg)
            else:
                await ctx.send(msg)
        ...
        embed = self._build_horoscope_embed(match, text, stats, sign_data)
