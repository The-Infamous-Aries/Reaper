...
class TicTacToeGame:
    ...
    def get_embed(self):
        ...
        for row in self.board:
            board_str += " | ".join([self.player1_emoji if c == self.X else self.player2_emoji if c == self.O else "⚪" for c in row]) + "\n"
...
class ShootingView(discord.ui.View):
    def __init__(self, target_position, user_id):
        ...
        hit_pe = discord.PartialEmoji(name="Hit", id=hit_id) if hit_id else "🎯"
        miss_pe = discord.PartialEmoji(name="Miss", id=miss_id) if miss_id else "🔴"
...
class RangeStatsView(discord.ui.View):
    ...
    def create_stats_embed(self):
        ...
        embed = discord.Embed(
            title=f"🎯 {self.user.display_name}'s DB4D Training Records",
...
class FunSystemCommands(commands.Cog):
    ...
    @commands.hybrid_command(name='range', description='Start DB4D Sniper Training')
    ...
    async def shooting_range(self, ctx, rounds: int = 10):
        ...
        if rounds not in valid_rounds:
            embed = discord.Embed(
                title="🎯 DB4D Sniper Training - Invalid Selection",
        ...
        if ctx.author.id in self.active_games:
            embed = discord.Embed(
                ...
                description="You already have an active DB4D sniper training session!",
                color=0xff9900
            )
            await ctx.send(embed=embed, ephemeral=True)
            return
    ...
    async def _run_shooting_range(self, ctx, rounds):
        ...
        setup_embed = discord.Embed(
            title="💀 DB4D Sniper Training Initializing...",
        ...
        for i in range(3, 0, -1):
            countdown_embed = discord.Embed(
                title=f"🎯 DB4D Training starts in {i}...",
    ...
    async def _display_results(self, ctx, hits, total_shots, accuracy, rounds, user_stats):
        # DB4D rank mapping: only 80%+ is positive; below 80% is negative with worsening titles
        if accuracy == 100:
        ...
        results_embed = discord.Embed(
            title="🎯 DB4D Sniper Training Results" + (" 🆕 NEW PERSONAL BEST!" if is_new_best else ""),
            color=rank_color
        )
