...
class ShowCog(commands.Cog):
    ...
    async def create_comprehensive_nation_embed(self, nation: Dict[str, Any]) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"{emoji_mod.mention('Infra') or '🏛️'} {nation_name}",
        ...
        embed.add_field(name=f"{emoji_mod.mention('Info') or '📊'} Basic Statistics", value=basic_stats, inline=False)
        ...
        if self.calculator:
            try:
                if self.calculator.has_project(nation, 'Missile Launch Pad'):
                    specializations.append(f"{emoji_mod.mention('Missile') or '🚀'} Missile")
                if self.calculator.has_project(nation, 'Nuclear Research Facility'):
                    specializations.append(f"{emoji_mod.mention('Nuke') or '☢️'} Nuke")
            except Exception:
                pass
        ...
        if safe_money_looted >= 2_500_000_000:
            specializations.append(f"{emoji_mod.mention('Pirate') or '🏴‍☠️'} Taxman")
        elif safe_money_looted >= 1_000_000_000:
            specializations.append(f"{emoji_mod.mention('Pirate') or '🏴'} Pirate")
        elif safe_money_looted >= 750_000_000:
            specializations.append(f"{emoji_mod.mention('Pirate') or '☠️'} Pillager")
        elif safe_money_looted >= 500_000_000:
            specializations.append(f"{emoji_mod.mention('Pirate') or '💀'} Bandit")
        elif safe_money_looted >= 250_000_000:
            specializations.append(f"{emoji_mod.mention('Money') or '💰'} Thief")
        elif safe_money_looted >= 100_000_000:
            specializations.append(f"{emoji_mod.mention('Money') or '💳'} Scammer")
        
        # War specializations
        if wars_won >= 500:
            specializations.append(f"{emoji_mod.mention('Soldier') or '🪦'} Reaper")
        elif wars_won >= 250:
            specializations.append(f"{emoji_mod.mention('Soldier') or '⚰️'} Murderer")
        elif wars_won >= 100:
            specializations.append(f"{emoji_mod.mention('Soldier') or '⚱️'} Fighter")
        
        # Credit specializations
        if safe_credits >= 5:
            specializations.append(f"{emoji_mod.mention('Info') or '🧠'} Planner")
        elif safe_credits >= 1:
            specializations.append(f"{emoji_mod.mention('Info') or '🤔'} Thinker")
        
        # Spy specializations
        if safe_spies >= 60:
            specializations.append(f"{emoji_mod.mention('Spy') or '🥷'} Shadow")
        
        # Reputation specializations
        commendations = nation.get('commendations', 0)
        if commendations >= 500:
            specializations.append(f"{emoji_mod.mention('Alert') or '🙇'} Worshipped")
        elif commendations >= 200:
            specializations.append(f"{emoji_mod.mention('Alert') or '🦸'} Idolized")
        elif commendations >= 100:
            specializations.append(f"{emoji_mod.mention('Alert') or '❤️'} Loved")
        elif commendations >= 50:
            specializations.append(f"{emoji_mod.mention('Alert') or '👍'} Liked")
        
        denouncements = nation.get('denouncements', 0)
        if denouncements >= 500:
            specializations.append(f"{emoji_mod.mention('Alert') or '🖕'} Despised")
        elif denouncements >= 200:
            specializations.append(f"{emoji_mod.mention('Alert') or '🦹'} Nemesis")
        elif denouncements >= 100:
            specializations.append(f"{emoji_mod.mention('Alert') or '💔'} Hated")
        elif denouncements >= 50:
            specializations.append(f"{emoji_mod.mention('Alert') or '👎'} Disliked")
            ...
            try:
                ...
                if military_analysis:
                    if military_analysis.get('has_ground_advantage', False):
                        specializations.append(f"{emoji_mod.mention('LandSup') or '🪖'} Ground")
                    if military_analysis.get('has_air_advantage', False):
                        specializations.append(f"{emoji_mod.mention('AirSup') or '✈️'} Air")
                    if military_analysis.get('has_naval_advantage', False):
                        specializations.append(f"{emoji_mod.mention('NavalSup') or '🚢'} Naval")
            except Exception:
                pass
        
        specialization_text = "\n".join(specializations) if specializations else f"{emoji_mod.mention('Soldier') or '⚔️'} Standard"
        ...
        embed.add_field(name=f"{emoji_mod.mention('Soldier') or '⚔️'} War Stats", value=military_info, inline=False)
        ...
        embed.add_field(
            name=f"{emoji_mod.mention('Soldier') or '⚔️'} Military Units",
            value=(
                f"{emoji_mod.mention('LandSup') or '🪖'} **Soldiers:** {soldiers_cur:,}/{limits.get('soldiers_max', 0):,}\n"
                f"{emoji_mod.mention('Tank') or '🚙'} **Tanks:** {tanks_cur:,}/{limits.get('tanks_max', 0):,}\n"
                f"{emoji_mod.mention('AirSup') or '🛩️'} **Aircraft:** {aircraft_cur:,}/{limits.get('aircraft_max', 0):,}\n"
                f"{emoji_mod.mention('NavalSup') or '⚓'} **Ships:** {ships_cur:,}/{limits.get('ships_max', 0):,}"
            ),
            inline=False
        )

        embed.add_field(
            name=f"{emoji_mod.mention('Project') or '🏭'} Daily Production",
            value=(
                f"{emoji_mod.mention('LandSup') or '🪖'} **Soldiers:** {limits.get('soldiers_daily', 0):,}/day\n"
                f"{emoji_mod.mention('Tank') or '🚙'} **Tanks:** {limits.get('tanks_daily', 0):,}/day\n"
                f"{emoji_mod.mention('AirSup') or '🛩️'} **Aircraft:** {limits.get('aircraft_daily', 0):,}/day\n"
                f"{emoji_mod.mention('NavalSup') or '⚓'} **Ships:** {limits.get('ships_daily', 0):,}/day\n"
                f"{emoji_mod.mention('Missile') or '🚀'} **Missiles:** {limits.get('missiles', 0):,}/day\n"
                f"{emoji_mod.mention('Nuke') or '☢️'} **Nukes:** {limits.get('nukes', 0):,}/day"
            ),
            inline=False
        )
        ...
        try:
            if self.calculator:
                project_categories = {
                    f"{emoji_mod.mention('Soldier') or '⚔️'} War": [
                    ...
                    f"{emoji_mod.mention('Project') or '🏭'} Industry": [
                    ...
                    f"{emoji_mod.mention('Infra') or '🏛️'} Government": [
                ...
                for category_key, projects in project_categories.items():
                    ...
                    if category_projects:
                        ...
                        category_mapping = {
                            (emoji_mod.mention('Soldier') or '⚔️'): 'War',
                            (emoji_mod.mention('Project') or '🏭'): 'Industry',
                            (emoji_mod.mention('Infra') or '🏛️'): 'Government',
                            (emoji_mod.mention('Info') or '👽'): 'Alien'
                        }
...
class NationSearchView(discord.ui.View):
    ...
    @discord.ui.button(label="Military", style=discord.ButtonStyle.secondary, emoji=emoji_mod.get_partial('Project') or "🏭")
    ...
    @discord.ui.button(label="Improvements", style=discord.ButtonStyle.secondary, emoji=emoji_mod.get_partial('Infra') or "🏗️")
...
class SearchNationMilitaryView(discord.ui.View):
    ...
    async def generate_nation_military_embed(self):
        try:
            ...
            embed = discord.Embed(
                title=f"{emoji_mod.mention('Project') or '🏭'} Military Analysis - {self.nation.get('nation_name', 'Unknown')}",
            ...
            embed.add_field(
                ...
                value=(
                    f"{emoji_mod.mention('LandSup') or '🪖'} **Soldiers:** {soldiers_cur:,}/{limits.get('soldiers_max', 0):,}\n"
                    f"{emoji_mod.mention('Tank') or '🛡️'} **Tanks:** {tanks_cur:,}/{limits.get('tanks_max', 0):,}\n"
                    f"{emoji_mod.mention('AirSup') or '✈️'} **Aircraft:** {aircraft_cur:,}/{limits.get('aircraft_max', 0):,}\n"
                    f"{emoji_mod.mention('NavalSup') or '🚢'} **Ships:** {ships_cur:,}/{limits.get('ships_max', 0):,}"
                ),
                inline=False
            ...
            embed.add_field(
                ...
                value=(
                    f"{emoji_mod.mention('LandSup') or '🪖'} **Soldiers:** {limits.get('soldiers_daily', 0):,}/day\n"
                    f"{emoji_mod.mention('Tank') or '🛡️'} **Tanks:** {limits.get('tanks_daily', 0):,}/day\n"
                    f"{emoji_mod.mention('AirSup') or '✈️'} **Aircraft:** {limits.get('aircraft_daily', 0):,}/day\n"
                    f"{emoji_mod.mention('NavalSup') or '🚢'} **Ships:** {limits.get('ships_daily', 0):,}/day\n"
                    f"{emoji_mod.mention('Missile') or '🚀'} **Missiles:** {limits.get('missiles', 0):,}/day\n"
                    f"{emoji_mod.mention('Nuke') or '☢️'} **Nukes:** {limits.get('nukes', 0):,}/day"
            ...
            adv_value = (
                f"{emoji_mod.mention('Missile') or '🚀'} **Missiles:** {missiles_cur:,} {'(Project)' if can_missile else ''}\n"
                f"{emoji_mod.mention('Nuke') or '☢️'} **Nukes:** {nukes_cur:,} {'(Project)' if can_nuke else ''}"
            )
            embed.add_field(name=f"{emoji_mod.mention('Missile') or '🚀'} Advanced Military", value=adv_value, inline=False)
            ...
            if adv_text_parts:
                embed.add_field(name=f"{emoji_mod.mention('Soldier') or '🎯'} Military Advantage", value="\n".join(adv_text_parts), inline=False)
...
class SearchNationImprovementsView(discord.ui.View):
    ...
    async def generate_nation_improvements_embed(self):
        try:
            ...
            embed = discord.Embed(
                title=f"{emoji_mod.mention('Infra') or '🏗️'} Improvements Breakdown - {self.nation.get('nation_name', 'Unknown Nation')}",
            ...
            embed.add_field(name=f"{emoji_mod.mention('Power') or '⚡'} Power", value=power_value, inline=True)
            ...
            if resource_improvements:
                embed.add_field(name=f"{emoji_mod.mention('Resources') or '⛏️'} Resources", value="\n".join(resource_improvements), inline=False)
            ...
            if manufacturing_improvements:
                embed.add_field(name=f"{emoji_mod.mention('Project') or '🏭'} Manufacturing", value="\n".join(manufacturing_improvements), inline=False)
            ...
            if military_improvements:
                embed.add_field(name=f"{emoji_mod.mention('Soldier') or '⚔️'} Military", value="\n".join(military_improvements), inline=False)
            ...
            if civil_improvements:
                embed.add_field(name=f"{emoji_mod.mention('Infra') or '🏢'} Civil", value="\n".join(civil_improvements), inline=False)
