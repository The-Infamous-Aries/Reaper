...
class RevenueCommand(commands.Cog):
    ...
    async def _create_nation_revenue_embed(self, nation_data: Dict[str, Any], revenue_data: Dict[str, Any]) -> discord.Embed:
        ...
        if resource_text:
            embed.add_field(
                name=f"{emoji_mod.mention('Raw') or '⛏️'} Raw Resource Production",
        ...
        if manufacturing_text:
            embed.add_field(
                name=f"{emoji_mod.mention('Manuf') or '🏭'} Manufacturing Production",
        ...
        if trade_prices:
            ...
            if price_text:
                embed.add_field(
                    name=f"{emoji_mod.mention('market') or '💹'} Current Trade Prices",
        ...
        if projects:
            if 'Arable Land Agency' in projects:
                bonuses_text += f"{emoji_mod.mention('food') or '🌾'} **Arable Land Agency:** +5% food production\n"
            if 'Advanced Engineering Corps' in projects:
                bonuses_text += f"{emoji_mod.mention('Manuf') or '⚙️'} **Advanced Engineering Corps:** +10% manufacturing\n"
            if 'Center Civil Engineering' in projects:
                bonuses_text += f"{emoji_mod.mention('Infra') or '🏗️'} **Center Civil Engineering:** +5% manufacturing\n"
        
        if bonuses_text:
            embed.add_field(
                name=f"{emoji_mod.mention('Info') or '✨'} Production Bonuses",
                value=bonuses_text,
                inline=False
            )
        ...
        if military_upkeep > 0:
            ...
            if soldiers > 0:
                upkeep_text += f"{emoji_mod.mention('LandSup') or '🪖'} Soldiers: {soldiers:,}\n"
            if tanks > 0:
                upkeep_text += f"{emoji_mod.mention('LandSup') or '🚙'} Tanks: {tanks:,}\n"
            if aircraft > 0:
                upkeep_text += f"{emoji_mod.mention('AirSup') or '✈️'} Aircraft: {aircraft:,}\n"
            if ships > 0:
                upkeep_text += f"{emoji_mod.mention('NavySup') or '⚓'} Ships: {ships:,}\n"
            if missiles > 0:
                upkeep_text += f"{emoji_mod.mention('munitions') or '🚀'} Missiles: {missiles:,}\n"
            if nukes > 0:
                upkeep_text += f"{emoji_mod.mention('uranium') or '☢️'} Nukes: {nukes:,}\n"
            
            upkeep_text += f"**Daily Upkeep:** ${military_upkeep:,.2f}"
            
            embed.add_field(
                name=f"{emoji_mod.mention('Attack') or '⚔️'} Military Upkeep",
                value=upkeep_text,
                inline=False
            )
        ...
        total_text = f"**{emoji_mod.mention('Gold') or '💰'} Total Revenue Summary:**\n"
        ...
        if raw_value > 0 or manufacturing_value > 0:
            total_text += f"**{emoji_mod.mention('Inventory') or '📦'} Resources:** ${total_resource_value:,.2f}/day\n"
            if raw_value > 0:
                total_text += f"**{emoji_mod.mention('Raw') or '⛏️'} Raw:** ${raw_value:,.2f}/day\n"
            if manufacturing_value > 0:
                total_text += f"**{emoji_mod.mention('Manuf') or '🏭'} Manufacturing:** ${manufacturing_value:,.2f}/day\n"
        ...
        embed.add_field(
            name=f"{emoji_mod.mention('Stat') or '📊'} Total Revenue Value",
            value=total_text,
            inline=False
        )
    ...
    async def _create_alliance_revenue_embed(self, alliance_data: Dict[str, Any], revenue_data: Dict[str, Any]) -> discord.Embed:
        ...
        title = f"{emoji_mod.mention('Alliance') or '🏛️'} Alliance Revenue - {alliance_name}"
        if acronym:
            title += f" [{acronym}]"
        ...
        # Total revenue
        embed.add_field(
            name=f"{emoji_mod.mention('Gold') or '💰'} Total Alliance Revenue",
        ...
        for color, count in revenue_data['color_breakdown'].items():
            color_emoji = emoji_mod.mention('Info') or '🎨'
            color_text += f"{color_emoji} **{color.capitalize()}:** {count} nations\n"
        
        if color_text:
            embed.add_field(
                name=f"{emoji_mod.mention('Info') or '🎨'} Color Distribution",
        ...
        if revenue_data['nation_revenues']:
            ...
            embed.add_field(
                name=f"{emoji_mod.mention('Ranking') or '🏆'} Top Revenue Nations",
                value=top_text,
                inline=True
            )
    ...
    async def _send_usage_embed(self, ctx):
        ...
        embed = discord.Embed(
            title=f"{emoji_mod.mention('Gold') or '💰'} Revenue Command Usage",
            description="Calculate revenue for nations and alliances",
            color=discord.Color.blue()
        )
        
        embed.add_field(
            name=f"{emoji_mod.mention('Alliance') or '🏛️'} Nation Revenue (Slash Command)",
            value="`/revenue query_type:Nation query_value:Test Nation`\n`/revenue query_type:Nation query_value:12345`",
            inline=False
        )
        
        embed.add_field(
            name=f"{emoji_mod.mention('Alliance') or '🤝'} Alliance Revenue (Slash Command)",
            value="`/revenue query_type:Alliance query_value:Test Alliance`\n`/revenue query_type:Alliance query_value:567`",
            inline=False
        )
        
        embed.add_field(
            name=f"{emoji_mod.mention('Alliance') or '🏛️'} Nation Revenue (Prefix Command)",
            value="`!revenue nation <name_or_id>`\nExample: `!revenue nation Test Nation`\nExample: `!revenue nation 12345`",
            inline=False
        )
        
        embed.add_field(
            name=f"{emoji_mod.mention('Alliance') or '🤝'} Alliance Revenue (Prefix Command)",
            value="`!revenue alliance <name_or_id>`\nExample: `!revenue alliance Test Alliance`\nExample: `!revenue alliance 567`",
            inline=False
        )
        
        embed.add_field(
            name=f"{emoji_mod.mention('Warning') or '⚠️'} Important Notes",
    ...
    async def _send_revenue_usage_embed(self, ctx):
        ...
        embed = discord.Embed(
            title=f"{emoji_mod.mention('Gold') or '💰'} Revenue Command Help",
            description="Complete guide for revenue calculations",
            color=discord.Color.blue()
        )
        
        embed.add_field(
            name=f"{emoji_mod.mention('Search') or '🔍'} Slash Commands (Recommended)",
        ...
        embed.add_field(
            name=f"{emoji_mod.mention('Info') or '📊'} What You'll See",
        ...
        embed.add_field(
            name=f"{emoji_mod.mention('Settings') or '💡'} Tips",
        ...
        embed.add_field(
            name=f"{emoji_mod.mention('Alert') or '🆘'} Need More Help?",
    ...
    def _create_nation_costs_embed(self, nation_name: str, results: Dict[str, Any], current_infra: float, current_land: float, current_cities: int, target_infra: float, target_land: float, nation_projects: set, specified_projects: set) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"{emoji_mod.mention('Infra') or '🏛️'} {nation_name} - Development Costs",
        ...
        # Add nation overview
        embed.add_field(
            name=f"{emoji_mod.mention('Info') or '📊'} Current Status",
        ...
        if nation_projects:
            embed.add_field(
                name=f"{emoji_mod.mention('Project') or '🏗️'} Nation's Active Projects",
        ...
        if 'infrastructure' in results and "error" not in results['infrastructure']:
            ...
            embed.add_field(
                name=f"{emoji_mod.mention('Project') or '🏗️'} Infrastructure Development",
        ...
        if 'land' in results and "error" not in results['land']:
            ...
            embed.add_field(
                name=f"{emoji_mod.mention('LandSup') or '🏕️'} Land Development",
        ...
        if 'next_city' in results and "error" not in results['next_city']:
            ...
            embed.add_field(
                name=f"{emoji_mod.mention('Nation') or '🏙️'} Next City Cost",
        ...
        if 'projects' in results and "error" not in results['projects']:
            ...
            if project_results.get("projects"):
                ...
                embed.add_field(
                    name=f"{emoji_mod.mention('Project') or '🏗️'} Project Costs Summary",
                ...
                for project in project_results['projects']:
                    if 'error' not in project:
                        ...
                        embed.add_field(
                            name=f"{emoji_mod.mention('Info') or '📋'} {project['project']}",
                            value=project_value,
                            inline=False
                        )
        ...
        if total_base_cost > 0:
            ...
            embed.add_field(
                name=f"{emoji_mod.mention('Money') or '💎'} Cost Summary",
        ...
        embed.set_footer(text=f"{emoji_mod.mention('Info') or 'ℹ️'} Costs show base prices vs potential policy savings. Only nation's existing projects apply discounts.")
        return embed
    ...
    async def _create_project_costs_embed(self, title: str, results: Dict[str, Any], domestic_policy: str) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"{emoji_mod.mention('Project') or '🏗️'} {title}",
        ...
        if domestic_policy:
            embed.add_field(
                name=f"{emoji_mod.mention('Info') or '📋'} Domestic Policy",
        ...
        if "error" in results:
            embed.add_field(
                name=f"{emoji_mod.mention('Alert') or '❌'} Error",
                value=results["error"],
                inline=False
            )
        else:
            ...
            for project in results["projects"]:
                if "error" in project:
                    embed.add_field(
                        name=f"{emoji_mod.mention('Alert') or '❌'} {project['project']}",
                        value=project["error"],
                        inline=False
                    )
                else:
                    # Monetary cost
                    project_details = f"{emoji_mod.mention('Money') or '💰'} **${project['monetary_cost']:,.2f}**"
                    ...
                    if project['resource_costs']:
                        ...
                        for resource, details in project['resource_costs'].items():
                            if details['price'] > 0:
                                emoji = self._get_resource_emoji(resource)
                                resource_details.append(f"{emoji} {details['amount']:,.0f} {resource} (${details['value']:,.2f})")
                            else:
                                emoji = self._get_resource_emoji(resource)
                                resource_details.append(f"{emoji} {details['amount']:,.0f} {resource} (price unavailable)")
                        
                        if resource_details:
                            project_details += f"\n{emoji_mod.mention('Crate') or '📦'} Resources: {', '.join(resource_details)}"
                    
                    # Total cost
                    project_details += f"\n{emoji_mod.mention('Money') or '💎'} **Total: ${project['total_cost']:,.2f}**"
                    
                    embed.add_field(
                        name=f"{emoji_mod.mention('Project') or '🏗️'} {project['project']}",
                        value=project_details,
                        inline=False
                    )
            
            # Add totals
            totals_text = f"{emoji_mod.mention('Money') or '💰'} Monetary: **${results['total_monetary_cost']:,.2f}**"
            if results['total_resource_value'] > 0:
                totals_text += f"\n{emoji_mod.mention('Crate') or '📦'} Resources: **${results['total_resource_value']:,.2f}**"
            totals_text += f"\n{emoji_mod.mention('Money') or '💎'} **Grand Total: ${results['total_cost']:,.2f}**"
            
            embed.add_field(
                name=f"{emoji_mod.mention('Info') or '📊'} Total Costs",
                value=totals_text,
                inline=False
            )
            ...
            if results['trade_prices']:
                ...
                for resource, price in results['trade_prices'].items():
                    if price > 0:
                        emoji = self._get_resource_emoji(resource)
                        price_info.append(f"{emoji} {resource}: ${price:.2f}")
                
                if price_info:
                    embed.add_field(
                        name=f"{emoji_mod.mention('Money') or '💹'} Current Trade Prices",
                        value=f"{', '.join(price_info[:5])}{'...' if len(price_info) > 5 else ''}",
                        inline=False
                    )
        
        embed.set_footer(text=f"{emoji_mod.mention('Info') or 'ℹ️'} Project costs calculated using current trade prices and domestic policy discounts")
        return embed
