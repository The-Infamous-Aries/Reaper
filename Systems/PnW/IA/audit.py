        ...
        try:
            ...
            if view_key == "resources":
                ...
                self._add_category_fields(
                    embed,
                    "Uranium < 1,000",
                    emoji_mod.mention('uranium') or "☢️",
                    uran_lt_500,
                    suffix_builder=lambda n: f"- {int(n.get('uranium', 0)):,}"
                )
                self._add_category_fields(embed, "Food = 0", emoji_mod.mention('Warning') or "🚫", food_zero)
                self._add_category_fields(embed, "Uranium = 0", emoji_mod.mention('Warning') or "🚫", uran_zero)
            elif view_key == "inactives":
                self._add_category_fields(embed, "Inactive 7-13 days", emoji_mod.mention('Neutral') or "⏲️", inactive_7_to_13, with_days=True)
                self._add_category_fields(embed, "Inactive 14-23 days", emoji_mod.mention('Warning') or "⚠️", inactive_14_to_23, with_days=True)
                self._add_category_fields(embed, "Inactive 24+ days", emoji_mod.mention('Alert') or "🛑", inactive_24_plus, with_days=True)
            elif view_key == "color":
                self._add_category_fields(embed, "Beige", emoji_mod.mention('Alert') or "🩼", beige)
                self._add_category_fields(embed, "Grey", emoji_mod.mention('Neutral') or "⚪", grey)
                self._add_category_fields(embed, "Wrong Color", emoji_mod.mention('Warning') or "🎨", wrong_color)
