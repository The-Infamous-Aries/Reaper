...
class CompareCog(commands.Cog):
    ...
    def _format_military_inline(self, stats: Dict[str, Any]) -> str:
        ...
        return (
            f"{emoji_mod.mention('LandSup') or '🪖'}: {tm.get('soldiers', 0):,}\n"
            f"{emoji_mod.mention('LandSup') or '🚙'}: {tm.get('tanks', 0):,}\n"
            f"{emoji_mod.mention('AirSup') or '🛩️'}: {tm.get('aircraft', 0):,}\n"
            f"{emoji_mod.mention('NavySup') or '⚓'}: {tm.get('ships', 0):,}\n"
            f"{emoji_mod.mention('munitions') or '🚀'}: {tm.get('missiles', 0):,}\n"
            f"{emoji_mod.mention('uranium') or '☢️'}: {tm.get('nukes', 0):,}"
        )
    ...
    def _make_comparison_embed(
        ...
        if (parties_home or []) or (parties_away or []):
            ...
            if len(home_list) > 1 or len(away_list) > 1:
                ...
                embed.add_field(name=f"{emoji_mod.mention('IA') or '🏛️'} Parties", value=parties_value, inline=False)
        ...
        embed.add_field(name=f"{emoji_mod.mention('Stat') or '📊'} Totals", value=totals_block, inline=False)
        ...
        embed.add_field(name=f"{emoji_mod.mention('Land') or '🏙️'} City Distribution", value=city_block, inline=False)
        ...
        mil_rows = [
            (emoji_mod.mention('LandSup') or "🪖", f"{tm_a.get('soldiers', 0):,}", f"{tm_b.get('soldiers', 0):,}"),
            (emoji_mod.mention('LandSup') or "🚙", f"{tm_a.get('tanks', 0):,}", f"{tm_b.get('tanks', 0):,}"),
            (emoji_mod.mention('AirSup') or "🛩️", f"{tm_a.get('aircraft', 0):,}", f"{tm_b.get('aircraft', 0):,}"),
            (emoji_mod.mention('NavySup') or "⚓", f"{tm_a.get('ships', 0):,}", f"{tm_b.get('ships', 0):,}"),
            (emoji_mod.mention('munitions') or "🚀", f"{tm_a.get('missiles', 0):,}", f"{tm_b.get('missiles', 0):,}"),
            (emoji_mod.mention('uranium') or "☢️", f"{tm_a.get('nukes', 0):,}", f"{tm_b.get('nukes', 0):,}")
        ]
        mil_block = self._make_unit_columns_block("Unit", left_hdr, right_hdr, mil_rows)
        embed.add_field(name=f"{emoji_mod.mention('MA') or '⚔️'} Current Military", value=mil_block, inline=False)

        # 4) Daily Purchases: show daily purchase capacity per unit
        dm_a = target_mill or {}
        dm_b = against_mill or {}
        daily_rows = [
            (emoji_mod.mention('LandSup') or "🪖", f"{int(dm_a.get('daily_soldiers', 0)):,}", f"{int(dm_b.get('daily_soldiers', 0)):,}"),
            (emoji_mod.mention('LandSup') or "🚙", f"{int(dm_a.get('daily_tanks', 0)):,}", f"{int(dm_b.get('daily_tanks', 0)):,}"),
            (emoji_mod.mention('AirSup') or "🛩️", f"{int(dm_a.get('daily_aircraft', 0)):,}", f"{int(dm_b.get('daily_aircraft', 0)):,}"),
            (emoji_mod.mention('NavySup') or "⚓", f"{int(dm_a.get('daily_ships', 0)):,}", f"{int(dm_b.get('daily_ships', 0)):,}"),
            (emoji_mod.mention('munitions') or "🚀", f"{int(dm_a.get('daily_missiles', 0)):,}", f"{int(dm_b.get('daily_missiles', 0)):,}"),
            (emoji_mod.mention('uranium') or "☢️", f"{int(dm_a.get('daily_nukes', 0)):,}", f"{int(dm_b.get('daily_nukes', 0)):,}")
        ]
        daily_block = self._make_unit_columns_block("Daily", left_hdr, right_hdr, daily_rows)
        embed.add_field(name=f"{emoji_mod.mention('Info') or '🔁'} Daily Purchases", value=daily_block, inline=False)
