...
def _create_treasure_trades_embed(trades_data: List[Dict[str, Any]], current_page: int, total_pages: int, show_canceled: bool = True) -> discord.Embed:
    """Creates a rich embed to display treasure trade information."""
    embed = discord.Embed(
        title="📉 Treasure Trades",
        description=f"Showing {'canceled/rejected' if show_canceled else 'all'} treasure trades.",
        color=discord.Color.orange(),
        timestamp=datetime.utcnow()
    )

    if not trades_data:
        embed.description = f"No {'canceled ' if show_canceled else ''}treasure trades found for this page."
        return embed

    for trade in trades_data:
        treasure_name = trade.get('treasure', 'Unknown Treasure')
        bonus = trade.get('bonus', 0)
        trade_amount = trade.get('money', 0)
        
        # Trade date formatting
        trade_date_str = trade.get('accept_date') or trade.get('offer_date')
        trade_display = 'Unknown'
        if trade_date_str:
            try:
                trade_dt = datetime.fromisoformat(trade_date_str.replace('Z', '+00:00'))
                trade_display = f'<t:{int(trade_dt.timestamp())}:R>'
            except (ValueError, TypeError):
                pass

        # Check if trade was canceled/rejected
        is_canceled = trade.get('rejected') or trade.get('seller_cancelled')
        is_buying = trade.get('buying', False)
        is_selling = trade.get('selling', False)
        
        if is_canceled:
            # Show declined trade format
            ...
            sender_name = trade.get('sender', {}).get('nation_name', 'Unknown')
            sender_id = trade.get('sender', {}).get('id')
            if not sender_id:
                sender_id = trade.get('sender_id')
            sender_link = f"[{sender_name}](https://politicsandwar.com/nation/id={sender_id})" if sender_id else sender_name
            
            trade_type = "buying" if is_buying else "selling"
            bonus_text = f"**Bonus:** +{bonus}\n" if bonus != 0 else ""
            field_value = (
                f"{bonus_text}"
                f"**Holder:** {holder_link}\n"
                f"**Declined {trade_type.title()} Trade:** ${trade_amount:,} from {sender_link} ({trade_display})"
            )
        else:
            # Show completed trade format for both buying and selling
            sender_name = trade.get('sender', {}).get('nation_name', 'Unknown')
            sender_id = trade.get('sender', {}).get('id')
            if not sender_id:
                sender_id = trade.get('sender_id')
            ...
            if not receiver_id:
                receiver_id = trade.get('receiver_id')
            receiver_link = f"[{receiver_name}](https://politicsandwar.com/nation/id={receiver_id})" if receiver_id else receiver_name
            
            bonus_text = f" with +{bonus}" if bonus != 0 else ""
            if is_buying:
                field_value = f"{receiver_link} bought {treasure_name}{bonus_text} for ${trade_amount:,} from {sender_link} ({trade_display})"
            else:
                field_value = f"{sender_link} sold {treasure_name}{bonus_text} for ${trade_amount:,} to {receiver_link} ({trade_display})"
...
class Finder(commands.Cog):
    ...
    @commands.hybrid_command(name="treasure_trades", description="Find recent treasure trades with filtering options.")
    @discord.app_commands.describe(
        score="Your nation score to filter by war range (shows trades within war range)",
        canceled="Toggle to show only canceled/rejected trades (On/Off)",
        limit="The number of trades to return (default: 10, max: 50)"
    )
    @discord.app_commands.choices(
        canceled=[
            discord.app_commands.Choice(name="On (Canceled Only)", value="on"),
            discord.app_commands.Choice(name="Off (All Trades)", value="off"),
        ]
    )
    async def treasure_trades(self, ctx: commands.Context, score: Optional[float] = None, canceled: str = "on", limit: int = 10):
        """Find and display treasure trades with filtering options."""
        await ctx.defer()

        # Validate limit parameter
        if limit < 1:
            await ctx.send("Limit must be at least 1.", ephemeral=True)
            return
        if limit > 50:
            await ctx.send("Limit cannot exceed 50.", ephemeral=True)
            return

        try:
            # Fetch trades with limit
            all_trades = await self.query.get_treasure_trades(limit=limit)
            if not all_trades:
            ...
            show_canceled_only = (canceled.lower() == "on")
            if show_canceled_only:
                # Show trades that were canceled/rejected (both buying and selling)
                filtered_trades = [
                    t for t in all_trades 
                    if (t.get('buying') or t.get('selling')) and (t.get('rejected') or t.get('seller_cancelled'))
                ]
            else:
                # Show all trades (both buying and selling)
                filtered_trades = [t for t in all_trades if t.get('buying') or t.get('selling')]

            # Filter by war range if score parameter provided
            if score:
                min_war_score = score * 0.75  # -25%
                max_war_score = score * 2.5   # +150%
                filtered_trades = [
                    t for t in filtered_trades 
                    if min_war_score <= (t.get('receiver', {}).get('score', 0) or 0) <= max_war_score
                ]

            if not filtered_trades:
                filter_desc = []
                if score:
                    filter_desc.append(f"war range ({score * 0.75:,.0f} - {score * 2.5:,.0f} score)")
                if limit != 10:
                    filter_desc.append(f"limit of {limit} trades")
                if show_canceled_only:
                    filter_desc.append("canceled trades only")
                
                filter_text = " and ".join(filter_desc) if filter_desc else "specified criteria"
                await ctx.send(f"No treasure trades found with {filter_text}.", ephemeral=True)
                return
            ...
            filtered_trades.sort(key=get_trade_date, reverse=True)

            view = PaginationView(filtered_trades, _create_treasure_trades_embed, per_page=5, show_canceled=show_canceled_only)
            initial_embed = await view.get_page_embed()
            await ctx.send(embed=initial_embed, view=view)
