...
class WarsNetBD(commands.Cog):
    ...
    @staticmethod
    def _extract_war_nation_name(war: dict, side: str) -> Optional[str]:
        nation_obj = war.get(side)
        if nation_obj and isinstance(nation_obj, dict):
            nation_name = nation_obj.get('nation_name')
            if nation_name:
                return nation_name

        flat_name_key = f"{side[:3]}_nation_name"
        flat_name = war.get(flat_name_key)
        if flat_name:
            return flat_name
    ...
    async def _get_nation_breakdown(self, all_wars, alliance_id, opps_view, resource_prices):
        ...
        if opps_view:
            for war in all_wars:
                if str(war.get('def_alliance_id')) == str(alliance_id):
                    nid = war.get('att_id')
                    if nid:
                        nname = self._extract_war_nation_name(war, 'attacker') or f"Nation {nid}"
                        nation_ids.add(nid)
                        # Only update name if we have a real one, or if not yet set
                        if nid not in nation_names or nation_names[nid].startswith("Nation "):
                ...
                if str(war.get('att_alliance_id')) == str(alliance_id):
                    nid = war.get('def_id')
                    if nid:
                        nname = self._extract_war_nation_name(war, 'defender') or f"Nation {nid}"
                        nation_ids.add(nid)
                        if nid not in nation_names or nation_names[nid].startswith("Nation "):
                            nation_names[nid] = nname
                        nation_alliance_info[nid] = {
        ...
        for nation_id in nation_ids:
            ...
            offense_wars_count = sum(1 for w in nation_wars if str(w.get('att_id')) == str(nation_id))
            defense_wars_count = sum(1 for w in nation_wars if str(w.get('def_id')) == str(nation_id))
            raid_wars_count = sum(1 for w in nation_wars if str(w.get('war_type', '')).upper() in ('RAID', '2') or w.get('war_type') == 2)
            attrition_wars_count = sum(1 for w in nation_wars if str(w.get('war_type', '')).upper() in ('ATTRITION', '1') or w.get('war_type') == 1)

            def _is_win(w):
                wid = w.get('winner_id')
                return wid and str(wid) == str(nation_id)

            def _is_loss(w):
                wid = w.get('winner_id')
                if not wid or str(wid) == '0':
                    return False
                return str(wid) != str(nation_id)

            def _is_peace(w):
                return (not w.get('winner_id') or str(w.get('winner_id')) == '0') \
                    and w.get('att_peace') == 1 and w.get('def_peace') == 1

            def _is_draw(w):
                return (not w.get('winner_id') or str(w.get('winner_id')) == '0') \
                    and not (w.get('att_peace') == 1 and w.get('def_peace') == 1) \
                    and (w.get('turns_left') or 0) < 0

            wins_count   = sum(1 for w in nation_wars if _is_win(w))
            losses_count = sum(1 for w in nation_wars if _is_loss(w))
            peace_count  = sum(1 for w in nation_wars if _is_peace(w))
            draws_count  = sum(1 for w in nation_wars if _is_draw(w))
            ...
            if True:
                ...
                nation_breakdown[nation_id] = {
                    ...
                    'total_damages': (
                        sum(d['cost'] for d in team2_costs.get('units', {}).values() if isinstance(d, dict)) +
                        (team2_costs.get('consumption', {}).get('munitions', 0) * resource_prices['buy'].get('munitions', 0)) +
                        (team2_costs.get('consumption', {}).get('gasoline', 0) * resource_prices['buy'].get('gasoline', 0)) +
                        team2_costs.get('infra_lost_value', 0) +
                        team2_costs.get('improvements_lost', 0) +
                        team2_costs.get('loot_lost', 0) +
                        sum(team2_costs.get('resource_loot_lost', {}).values()) +
                        team2_costs.get('money_destroyed', 0)
                    ),
                    'wars_count': len(nation_wars),
                    'offense_wars_count': offense_wars_count,
                    'defense_wars_count': defense_wars_count,
                    'raid_wars_count': raid_wars_count,
                    'attrition_wars_count': attrition_wars_count,
                    'wins_count': wins_count,
                    'losses_count': losses_count,
                    'peace_count': peace_count,
                    'draws_count': draws_count,
                    'damages': team2_costs.get('gross', 0),
                    ...
                    # missiles/nukes_eaten = enemy fired AND hit us (infra_destroyed > 0)
                    'missiles_eaten': sum(
                        1 for w in nation_wars
                        for a in w.get('attacks', [])
                        if (a.get('att_missiles_lost') or 0) > 0
                        and (a.get('infra_destroyed') or 0) > 0
                        and str(a.get('def_id')) == str(nation_id)
                    ),
                    'nukes_eaten': sum(
                        1 for w in nation_wars
                        for a in w.get('attacks', [])
                        if (a.get('att_nukes_lost') or 0) > 0
                        and (a.get('infra_destroyed') or 0) > 0
                        and str(a.get('def_id')) == str(nation_id)
                    ),
                    # missiles/nukes_blocked = enemy fired but we blocked them (infra_destroyed == 0)
                    'missiles_blocked': sum(
                        1 for w in nation_wars
                        for a in w.get('attacks', [])
                        if (a.get('att_missiles_lost') or 0) > 0
                        and (a.get('infra_destroyed') or 0) == 0
                    ...
                    # missiles/nukes_missed = WE fired but enemy blocked (infra_destroyed == 0)
                    'missiles_missed': sum(
                        1 for w in nation_wars
                        for a in w.get('attacks', [])
                        if (a.get('att_missiles_lost') or 0) > 0
                        and (a.get('infra_destroyed') or 0) == 0
                        and str(a.get('att_id')) == str(nation_id)
                    ),
                    'nukes_missed': sum(
                        1 for w in nation_wars
                        for a in w.get('attacks', [])
                    ...
                    # missiles/nukes_hit = WE fired AND took infra (infra_destroyed > 0)
                    'missiles_hit': sum(
                        1 for w in nation_wars
                        for a in w.get('attacks', [])
                        if (a.get('att_missiles_lost') or 0) > 0
                    ...
                    # enemy units killed by us
                    'enemy_soldiers_killed': team2_costs.get('units', {}).get('soldiers', {}).get('lost', 0),
                    'enemy_tanks_killed': team2_costs.get('units', {}).get('tanks', {}).get('lost', 0),
                    'enemy_aircraft_killed': team2_costs.get('units', {}).get('aircraft', {}).get('lost', 0),
                    'enemy_ships_killed': team2_costs.get('units', {}).get('ships', {}).get('lost', 0),
                    'units_total_cost': team1_costs.get('units_total_cost', sum(
                        unit_data.get('cost', 0) for unit_data in team1_costs.get('units', {}).values()
                    ...
                    'enemy_infra_destroyed': team2_costs.get('infra_lost_levels', 0),
                    'enemy_infra_destroyed_value': team2_costs.get('infra_lost_value', 0),
    ...
    def _calculate_enemy_relationships(self, all_wars: List[dict], nation_breakdown: dict, alliance_id: str) -> Dict[int, Dict[int, Dict[str, Any]]]:
        ...
        for war in all_wars:
            ...
            if att_alliance_id == alliance_id:
                damage_dealt = (att_infra_destroyed * 1000) + (def_units_lost * 50)
    ...
    async def wars_net_breakdown(self, interaction: discord.Interaction, alliance: str, time: Optional[str] = None, force_refresh: bool = False, opps_view: bool = False):
        ...
        try:
            ...
            interactive_url = f"{public_url}/api/pnw/war_net?alliance={quote_plus(alliance)}&time={quote_plus(time or 'all')}&force_refresh={force_refresh}&opps_view={opps_view}"
            ...
            view = WarNetBreakdownView(embeds, None, leaderboard_embed)
