...
class WarsBD(commands.Cog):
    ...
    def _generate_alliance_cost_graph(self, nation_breakdown: Dict[int, Any], alliance_name: str, time_str: str, opps_view: bool, total_wars: int) -> Optional[io.BytesIO]:
        ...
        total_net = sum(d['net_cost'] for d in nation_details.values())
        total_net_abs = sum(abs(d['net_cost']) for d in nation_details.values())
        ...
        if total_net_abs > 0:
            ...
            for name, details in sorted_by_cost:
                net_value_abs = abs(details['net_cost'])
                if net_value_abs > 0:
        ...
        for i, (name, details) in enumerate(left_items):
            ...
            self.draw_legend_item(draw, current_x, current_y, box_size, line_height, column_width, color_map[name], name, details, total_cost, total_gains, total_net, label_font)
    ...
    def draw_legend_item(self, draw, x, y, box_size, line_height, column_width, color, name, details, total_cost, total_gains, total_net, font):
        ...
        net_contribution_percentage = (net_cost / total_net) * 100 if total_net != 0 else 0
        net_color = '#e74c3c' if net_cost > 0 else '#2ecc71'
        net_cost_str = f"Net: ${net_cost:,.0f} {{{net_contribution_percentage:.1f}%}}"
        draw.text((x + box_size + 10, y_offset), net_cost_str, font=font, fill=net_color)
    ...
    def _create_leaderboard_embed(self, nation_breakdown: dict, resource_prices: dict, alliance_name: str, opps_view: bool) -> discord.Embed:
        ...
        # 6. Best Net - using alliance emojis
    ...
    async def wars_breakdown(self, interaction: discord.Interaction, alliance: str, time: str, force_refresh: bool = False, opps_view: bool = False):
        ...
        try:
            ...
            war_graph_generator.set_public_url(public_url, self.port)
            ...
            view = WarBreakdownView(embeds, graph_file, leaderboard_embed)
