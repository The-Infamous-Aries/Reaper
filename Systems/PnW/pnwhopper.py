...
async def build_nation_mini_embed(
    ...
    basic_stats = (
        f"{emoji_mod.mention('Flag') or '🚩'} **Alliance:** {nation.get('alliance_name', 'None')}\n"
        f"{emoji_mod.mention('Trophy') or '🎖️'} **Position:** {str(nation.get('alliance_position', 'Unknown')).title()}\n"
        f"{emoji_mod.mention('Beach') or '🏖️'} **Vacation Mode:** {'Yes' if int(vacation_turns or 0) > 0 else 'No'}\n"
        f"{emoji_mod.mention('Art') or '🎨'} **Color:** {color_str.title()}\n"
        f"{beige_line}"
        f"{emoji_mod.mention('Chat') or '💬'} **Discord:** `{discord_info}`\n"
        f"{emoji_mod.mention('Loading') or '🕒'} **Last Active:** {last_active}"
    )
    embed.add_field(name=f"{emoji_mod.mention('Note') or '📊'} Basic Statistics", value=basic_stats, inline=False)
    
    growth_stats = (
        f"{emoji_mod.mention('Hammer') or '🏗️'} **New Project:** {project_status}\n"
        f"{emoji_mod.mention('City') or '🏙️'} **New City:** {city_status}\n"
        f"{emoji_mod.mention('Building') or '🏢'} **Total Cities:** `{int(nation.get('num_cities', 0))}`\n"
        f"{emoji_mod.mention('Charge') or '⚡'} **Powered:** `{int(powered_cities)}/{len(cities)}`"
    )
    embed.add_field(name=f"{emoji_mod.mention('Chart') or '📈'} Growth & Development", value=growth_stats, inline=True)
    
    infra_stats = (
        f"{emoji_mod.mention('Stairs') or '🪜'} **Tier:** {infra_tier}\n"
        f"{emoji_mod.mention('Ruler') or '📐'} **Total Infra:** `{float(total_infra):,.0f}`\n"
        f"{emoji_mod.mention('Straightedge') or '📏'} **Avg/City:** `{float(avg_city_infra):,.0f}`\n"
        f"{emoji_mod.mention('Balance') or '⚖️'} **Policy:** {nation.get('domestic_policy', 'Unknown')}"
    )
