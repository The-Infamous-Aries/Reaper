...
def create_interactive_comparison_page(home_individual_stats, away_individual_stats):
    ...
    home_alliance_names = "-".join(sorted([a['name'] for a in home_individual_stats]))
    away_alliance_names = "-".join(sorted([a['name'] for a in away_individual_stats]))
    html_filename = f"comparison_{home_alliance_names}_{away_alliance_names}_{today_str}.html"
    web_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', 'web', 'Comparisons')
    os.makedirs(web_dir, exist_ok=True)
    html_filepath = os.path.join(web_dir, html_filename)
    ...
    # Calculate military totals using the correct structure from AllianceCalculator
    home_military_totals = {}
    away_military_totals = {}
    
    for unit in stat_keys:
        for type_prefix in ['current', 'daily']:
            home_key = f"{type_prefix}_{stat_keys[unit]}"
            away_key = f"{type_prefix}_{stat_keys[unit]}"
            
            home_total = sum(ind['stats'].get('daily_military', {}).get(home_key, 0) for ind in home_individual_stats)
            away_total = sum(ind['stats'].get('daily_military', {}).get(away_key, 0) for ind in away_individual_stats)
            
            home_military_totals[f"{unit}_{type_prefix}"] = home_total
            away_military_totals[f"{unit}_{type_prefix}"] = away_total

    for unit in stat_keys:
        military_x_labels.append(unit)
        military_x_positions.append(mil_pos + 2.5)
        for type_prefix in ['current', 'daily']:
            max_val = max(home_military_totals.get(f"{unit}_{type_prefix}", 0), away_military_totals.get(f"{unit}_{type_prefix}", 0), 1)
            
            for ind in home_individual_stats:
                val = ind['stats'].get('daily_military', {}).get(f"{type_prefix}_{stat_keys[unit]}", 0)
                percent_of_max = (val / max_val) * 100
                side_total = home_military_totals.get(f"{unit}_{type_prefix}", 1)
                percent_of_side = (val / side_total) * 100 if side_total > 0 else 0
                hovertemplate = (
                    f"<b>{unit.capitalize()} ({type_prefix.capitalize()})</b><br><br>"
            ...
            for ind in away_individual_stats:
                val = ind['stats'].get('daily_military', {}).get(f"{type_prefix}_{stat_keys[unit]}", 0)
                percent_of_max = (val / max_val) * 100
                side_total = away_military_totals.get(f"{unit}_{type_prefix}", 1)
                percent_of_side = (val / side_total) * 100 if side_total > 0 else 0
                hovertemplate = (
                    f"<b>{unit.capitalize()} ({type_prefix.capitalize()})</b><br><br>"
                    "<b>Alliance:</b> %{data.name}<br>"
