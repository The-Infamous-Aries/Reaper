...
class PetSystem:
    ...
    def get_element_style(self, element: str, element2: Optional[str] = None) -> Tuple[int, str]:
        ...
        return final_color, LootCalculator.get_pet_emoji("Elements", element) or ""
    ...
    async def process_adoption(self, user_id: int, user_name: str, category: str, species_input: str, element1: str, element2: str, custom_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                + (f" • {LootCalculator.get_pet_emoji('Elements', pet_data.get('element2'))} {str(pet_data.get('element2')).title()}" if pet_data.get('element2') else "")
    ...
    async def create_pet_status_embed(self, user_id: int, pet_data: Optional[Dict[str, Any]] = None) -> discord.Embed:
        ...
        embed.add_field(name="⚔️ Fighting", value=f"{att_e} {fmt_stat('ATT')} {totals['ATT']} | {def_e} {fmt_stat('DEF')} {totals['DEF']}", inline=False)
        embed.add_field(name="🧪 Usage", value=f"{int_e} {fmt_stat('INT')} {totals['INT']} | {dex_e} {fmt_stat('DEX')} {totals['DEX']}", inline=False)
        embed.add_field(name="❤️ Life", value=f"{hap_e} {fmt_stat('HAP')} {totals['HAP']} | {ene_e} {fmt_stat('ENE')} {totals['ENE']}", inline=False)
        ...
        for cat in ["Hat", "Potion", "Material", "Gem", "Monster", "Key", "Chest"]:
            if cat in item_counts and item_counts[cat]:
                ...
                for name, count in item_counts[cat].most_common():
                    ...
                    em_id = emoji_mod.id_for(name)
                    em = emoji_mod.get(em_id) or ""
...
class PetShopView(discord.ui.View):
    ...
    async def adopt_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        select = discord.ui.Select(placeholder="Select which pet to adopt...", options=options)
        ...
        # Add a back button
        back_btn = discord.ui.Button(label="Cancel", style=discord.ButtonStyle.secondary)
...
class AdoptionConfigView(discord.ui.View):
    ...
    async def select_element1(self, interaction: discord.Interaction):
        if interaction.user.id != self.user.id:
            await interaction.response.send_message("This is not your session!", ephemeral=True)
            return
        self.element1 = interaction.data['values'][0]
    ...
    async def select_element2(self, interaction: discord.Interaction):
        if interaction.user.id != self.user.id:
            await interaction.response.send_message("This is not your session!", ephemeral=True)
            return
        self.element2 = interaction.data['values'][0]
        self.update_selects()
        await interaction.response.edit_message(embed=self.create_config_embed(), view=self)
...
class LootMarketView(discord.ui.View):
    def __init__(self, user_id: int, pet_data: Dict[str, Any]):
        ...
        self.chest_select = discord.ui.Select(
            ...
            options=[
                discord.SelectOption(label="Chest 1 (1 Item)", value="chest1", description="Cost: 1x Key1", emoji="📦"),
                discord.SelectOption(label="Chest 2 (2 Items)", value="chest2", description="Cost: 1x Key2", emoji="📦"),
                discord.SelectOption(label="Chest 3 (4 Items)", value="chest3", description="Cost: 1x Key3", emoji="📦"),
                discord.SelectOption(label="Chest 4 (8 Items)", value="chest4", description="Cost: 1x Key3", emoji="📦"),
            ],
            row=0
        )
