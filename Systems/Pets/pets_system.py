...
if TYPE_CHECKING:
    from discord.types.interactions import SelectMessageComponentInteractionData, SelectMenuInteractionData
...
class PetSystem:
    ...
    async def _maybe_trigger_wild_encounter(self, ctx: commands.Context, pet: Dict[str, Any]):
        """
        Check if a wild encounter should be triggered based on pet's level.
        """
        pet_level = pet.get('level', 1)
        # Scaling chance: 5% base + 0.2% per level, capped at 25%
        encounter_chance = min(0.25, 0.05 + (pet_level * 0.002))

        if random.random() < encounter_chance:
            from .PetGames.battle_system import battle_system

            # Notify the user
            await ctx.send(f"🌲 A wild creature appeared! {pet['name']} must defend itself!")

            # Create a wild encounter battle
            battle_view = await battle_system.create_battle(
                ctx=ctx,
                battle_type="solo",
                participants=[(ctx.author, pet)],
                interaction=ctx.interaction, # Pass interaction for followup messages
                wild_encounter=True
            )

            # Send the main battle message
            if battle_view:
                embed = battle_view.build_spectator_embed("The battle begins!")
    ...
    def _load_locations_data(self):
        ...
        logic_path = os.path.join(script_dir, "Logic", "locations_play.json")
        try:
            ...
            print(f"Error: Could not decode JSON from {logic_path}.")
            return {}

    def _generate_boss_for_location(self, pet: Dict[str, Any], location_name: str) -> Dict[str, Any]:
        """Generate a boss monster for a given location."""
        locations_data = self._load_locations_data()
        location_info = locations_data.get("locations", {}).get(location_name, {})
        location_elements = location_info.get("Special", {}).get("Elements", ["basic"])

        pet_stats = StatsCalculator.calculate_pet_stats(pet)
        pet_health = pet_stats.get('max_health', 500)
        pet_attack = pet_stats.get('attack', 10)
        pet_defense = pet_stats.get('defense', 5)

        boss_health = int(pet_health * (random.uniform(2.5, 4.0)) * (1 + pet.get('level', 1) / 10))
        boss_attack = int(pet_attack * random.uniform(0.6, 0.8))
        boss_defense = int(pet_defense * random.uniform(0.7, 0.9))

        boss_element1 = random.choice(location_elements)
        boss_element2 = None
        if len(location_elements) > 1:
            possible_elements = [e for e in location_elements if e != boss_element1]
            if possible_elements:
                boss_element2 = random.choice(possible_elements)

        boss_name = f"Guardian of {location_name}"
        ...
        return {
            ...
            "type": "boss",
            "element": boss_element1,
            "element2": boss_element2,
        }

    async def handle_boss_encounter(self, interaction: discord.Interaction, place_name: str) -> bool:
        """Handle the logic for a boss encounter."""
        from .PetGames.battle_system import battle_system, UnifiedBattleView
        ...
        boss_monster = self._generate_boss_for_location(pet, place_name)

        await interaction.response.send_message(f"🔥 A powerful **{boss_monster['name']}** appears! Prepare for battle!")

        battle_view = await UnifiedBattleView.create_async(
            ctx=interaction,
            battle_type="solo",
            participants=[(interaction.user, pet)],
            monster=boss_monster, # Pass the generated boss monster
            interaction=interaction,
            is_boss_battle=True
        )

        if battle_view:
            embed = battle_view.build_spectator_embed("A boss has appeared!")
    ...
    async def create_pet(self, user_id: int, user_name: str, category: str, species: str, element: Optional[str] = None, element2: Optional[str] = None, custom_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        await user_data_manager.save_pet_data(str(user_id), user_name, pet_data)
    ...
    async def create_pet_breakdown_embed(self, user_id: int, pet_data: Optional[Dict[str, Any]] = None) -> discord.Embed:
        ...
        # Battle Records
        battle_stats = pet.get("battle_stats", {})
        records_str = ""
        # PvP
        pvp_stats = battle_stats.get("pvp", {})
        records_str += f"PvP: {pvp_stats.get('wins', 0)} Wins, {pvp_stats.get('losses', 0)} Losses\n"
        # NPC
        npc_stats = battle_stats.get("npc", {})
        records_str += f"NPC: {npc_stats.get('wins', 0)} Wins, {npc_stats.get('losses', 0)} Losses\n"
        # Wild Encounter
        wild_stats = battle_stats.get("wild_encounter", {})
        records_str += f"Wild Encounters: {wild_stats.get('wins', 0)} Wins, {wild_stats.get('losses', 0)} Losses\n"
        # Boss
        boss_stats = battle_stats.get("boss", {})
        records_str += f"Bosses: {boss_stats.get('wins', 0)} Wins, {boss_stats.get('losses', 0)} Losses\n"

        if records_str:
            embed.add_field(name="Battle Records", value=records_str, inline=False)
    ...
    async def perform_mission(self, ctx: commands.Context, difficulty: str, gamble_xp: Optional[int] = None) -> Dict[str, Any]:
        ...
        pet = await self.get_user_pet(user_id)
        if not pet:
            return {"narrative": "You don't have a pet!", "level_up": None, "level_down": None, "wild_encounter": False}
        ...
        if success:
            ...
            # Mission failed
            if await self._maybe_trigger_wild_encounter(ctx, pet):
                return {"narrative": "", "level_up": None, "level_down": None, "wild_encounter": True}
        ...
        return {"narrative": narrative, "level_up": level_up, "level_down": None, "wild_encounter": False}
...
class KillConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await user_data_manager.delete_pet_data(str(self.ctx.author.id))
