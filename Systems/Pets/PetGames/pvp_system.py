...
class PvPBattleView(discord.ui.View):
    ...
    def __init__(self, ctx, participants: Union[List[discord.Member], Dict[str, List[discord.Member]]], 
        ...
        if battle_mode == BattleMode.FREE_FOR_ALL:
            self.teams = {str(i): [member] for i, member in enumerate(participants[:10])}
    ...
    async def process_attack_vs_defense(self, attacker_id: str, target_id: str, defender_id: str):
        ...
        result = DamageCalculator.calculate_battle_action(
            ...
            attacker_species=attacker.get('pet', {}).get('species'),
            defender_species=defender.get('pet', {}).get('species')
        )
        ...
        self.battle_log.append(
            f"⚔️ {attacker['user'].display_name} uses {atk_name} on {target['user'].display_name} "
            f"(Roll: {result['attack_roll']}, {result['attack_result']}) "
            f"🛡️ {defender['user'].display_name} uses {def_name} "
    ...
    async def process_undefended_attack(self, attacker_id: str, target_id: str):
        ...
        result = DamageCalculator.calculate_battle_action(
            ...
            attacker_species=attacker.get('pet', {}).get('species'),
            defender_species=target.get('pet', {}).get('species')
        )
    ...
    async def process_mutual_attack(self, player1_id: str, player2_id: str):
        ...
        result1 = DamageCalculator.calculate_battle_action(
            ...
            attacker_species=player1.get('pet', {}).get('species'),
            defender_species=player2.get('pet', {}).get('species')
        )
        ...
        result2 = DamageCalculator.calculate_battle_action(
            ...
            attacker_species=player2.get('pet', {}).get('species'),
            defender_species=player1.get('pet', {}).get('species')
        )
        ...
        self.battle_log.append(
            f"⚔️💥 MUTUAL STRIKES! {player1['user'].display_name} uses {a1} and {player2['user'].display_name} uses {a2} simultaneously!"
        )
        self.battle_log.append(
    ...
    def build_battle_embed(self, title: str) -> discord.Embed:
        ...
        for member_id, data in self.players.items():
            ...
            if data.get('charging'):
                status.append(f"⚡x{data.get('charge',1.0):.1f}")
    ...
    @staticmethod
    def _get_hp_bar(percentage: float, pet: dict = None, length: int = 10) -> str:
...
class PvPActionView(discord.ui.View):
    ...
    def add_buttons(self):
            ...
            # In FFA, show all other players as potential targets
                attack_options = [
                    discord.SelectOption(
                        ...
                        value=member_id,
                    emoji=discord.PartialEmoji(id=emoji_mod.id_for('Attack'))
                    )
        ...
        if attack_options:
            ...
            try:
                ...
                attack_ph = f"⚔️ {labels.get('attack','Attack')} • {t_name} (select target)"
            except Exception:
        ...
        defend_options = [discord.SelectOption(label="Yourself", value="self", emoji=discord.PartialEmoji(id=emoji_mod.id_for('Defend')))]
        ...
        try:
            ...
            t_emoji = type_emojis.get(ptype,'❓')
            t_name = (ptype or 'unknown').title()
            defend_ph = f"🛡️ {labels.get('defend','Defend')} • {t_name} (self only)"
        except Exception:
        ...
        try:
            ...
            charge_label = f"{e_emoji} {labels.get('charge','Charge')} x{current_charge}"
        except Exception:
            charge_label = "Charge"
        charge_button = discord.ui.Button(
            style=discord.ButtonStyle.success,
            label=charge_label,
            emoji=discord.PartialEmoji(id=emoji_mod.id_for('Charge')),
            custom_id="charge",
            row=1
        )
    ...
    async def charge_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            cverb = "Charge"
        self.battle_view.battle_log.append(f"⚡ {interaction.user.display_name} channels {cverb}! (Charge: x{new_charge})")
