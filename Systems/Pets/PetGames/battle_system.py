...
class UnifiedBattleView(discord.ui.View):
        ...
        for player_id, action_data in self.player_actions.items():
            if action_data['action'] == 'defend':
                ...
                if battle_result['attack_result'] == "miss":
                    ...
                    action_text += f"🛡️ **{player_name}** uses {dverb} with {roll_multiplier:.2f}x effectiveness! (Roll: {battle_result['attack_roll']}){charge_text}\n"
        ...
        for player_id, action_data in self.player_actions.items():
            if action_data['action'] == 'charge':
                ...
                try:
                    ...
                    verb = "Charging"
                action_text += f"⚡ **{player_name}** channels {verb}! (Charge: x{next_multiplier})\n"
            ...
            if action_data['action'] == 'attack' and action_data['target']:
                ...
                if parry_damage > 0:
                    ...
                    if battle_result['attack_result'] == "miss":
                        ...
                        action_text += f"⚔️ **{attacker_name}** uses {atk} on **{defender_name}**! (Roll: {battle_result['attack_roll']}, Multiplier: {roll_multiplier:.2f}x){charge_text}{defend_text} → {final_damage} damage dealt\n"
    ...
    def build_spectator_embed(self, action_text: str = "") -> discord.Embed:
        ...
        for user_id, data in self.player_data.items():
            ...
            defending_info = " 🛡️" if data.get('defending') else ""
    ...
    def __init__(self, battle_view: UnifiedBattleView, user_id: int):
        ...
        try:
            ...
            if hasattr(self, 'attack_button'):
                ...
                self.attack_button.emoji = discord.PartialEmoji(id=emoji_mod.id_for('Attack'))
            if hasattr(self, 'defend_button'):
                ...
                self.defend_button.emoji = discord.PartialEmoji(id=emoji_mod.id_for('Defend'))
            if hasattr(self, 'charge_button'):
                ...
                self.charge_button.emoji = discord.PartialEmoji(id=emoji_mod.id_for('Charge'))
