...
class RaceSession(discord.ui.View):
    def __init__(self, bot: commands.Bot, owner: discord.Member, simulation: bool, difficulty: Optional[str], mode_betting: bool, bet_amount: Optional[int]):
        ...
        self.segment_threshold = 50.0
        self.task: Optional[asyncio.Task] = None
        self._init_session()

    def _init_session(self):
        self.participants = [self.owner.id]
        self.tracks[self.owner.id] = ["➖"] * self.max_segments
    ...
    async def _setup_competitors(self) -> bool:
        if self.simulation:
            ...
            for _ in range(3):
                ...
                self.tracks[bot_id] = ["➖"] * self.max_segments
    ...
    def _race_embed(self) -> discord.Embed:
        ...
        for uid in self.participants:
            ...
            left = "➖" * pos
            right = "➖" * (self.max_segments - 1 - pos)
            track = f"{left}{em}{right}🏁"
            name = "Bot" if not self.bot.get_user(uid) else self.bot.get_user(uid).display_name
            pet = self.pets.get(uid, {})
            dex = int(pet.get("DEX", 0))
            ene = int(pet.get("ENE", 0))
            hap = int(pet.get("HAP", 0))
            total = int((dex + ene + hap) / 1000.0)
    ...
    async def _handle_player_io(self, uid: int, change: int, bet: int, win: bool) -> Optional[str]:
        ...
        async def _do_loot():
            if win and user:
                pet = await user_data_manager.get_pet_data_async(str(uid), name)
                if pet:
                    streak = self.win_streaks.get(uid, 1)
                    loot = await LootCalculator.award_gambling_loot(uid, pet, difficulty="normal", win_streak=streak, source="races")
                    if loot:
                        return f"\n💎 {user.mention} found loot! {', '.join(loot)}"
            return None
    ...
    async def _settle_bets(self):
        ...
        if io_tasks:
            ...
            for msg in results:
                if msg and self.message:
                    await self.message.channel.send(msg)

        # Show "Play Again" / Reset option
        if self.message:
            # We can re-enable the Play Again button here or send a new message
            pass
    ...
    async def join_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if len(self.participants) >= 4:
            ...
            return
        pet = await self._load_user_pet(interaction.user)
        if not pet:
        ...
        if self.mode_betting:
            modal = discord.ui.Modal(title="Enter Bet")
            bet_input = discord.ui.TextInput(label="Bet XP", placeholder="Enter XP", min_length=1, max_length=8)
            modal.add_item(bet_input)
            async def on_submit(inter: discord.Interaction):
                try:
                    amt = int(str(bet_input.value or "0").strip())
                except Exception:
                    await inter.response.send_message("Invalid bet.", ephemeral=True)
                    return
                ok = await _deduct_xp(interaction.user, amt, pet_data=pet)
                if not ok:
                    await inter.response.send_message("Bet exceeds available XP.", ephemeral=True)
                    return
                self.bets[interaction.user.id] = amt
                if self.message:
                    await self.message.edit(embed=self._lobby_embed(), view=self)
