...
class RoundControlView(ui.View):
    ...
    def _render_champion_image_sync(winner_name: str, eliminations: int, avatar_bytes: Optional[bytes], info: Dict[str, Any]) -> Optional[BytesIO]:
        ...
        try:
            ...
            element1 = str(info.get("element", "basic")).lower()
            element2 = str(info.get("element2", "")).lower()
            if not element2 or element2 == "none":
                element2 = None
            ...
            def draw_element_decorator(draw, el_name, start_deg, end_deg, step_deg=15):
                for ang in range(start_deg, end_deg, step_deg):
                    rad = math.radians(ang)
                    # Base anchoring point right off the avatar edge
                    bx = cx + (circle_r + 10) * math.cos(rad)
                    by = cy + (circle_r + 10) * math.sin(rad)
                    
                    if el_name == "fire":
                        tip_rad = math.radians(ang + 8)
                        tx = cx + (circle_r + 45) * math.cos(tip_rad)
                        ty = cy + (circle_r + 45) * math.sin(tip_rad)
                        lx = cx + (circle_r + 5) * math.cos(math.radians(ang - 6))
                        ly = cy + (circle_r + 5) * math.sin(math.radians(ang - 6))
                        rx = cx + (circle_r + 5) * math.cos(math.radians(ang + 6))
                        ry = cy + (circle_r + 5) * math.sin(math.radians(ang + 6))
                        color = (255, 80 + (ang % 50), 0, 220)
                        draw.polygon([(lx, ly), (tx, ty), (rx, ry)], fill=color)

                    elif el_name == "electric":
                        mrad = math.radians(ang + 10)
                        mx = cx + (circle_r + 25) * math.cos(mrad)
                        my = cy + (circle_r + 25) * math.sin(mrad)
                        tx = cx + (circle_r + 45) * math.cos(rad)
                        ty = cy + (circle_r + 45) * math.sin(rad)
                        draw.line([(bx, by), (mx, my), (tx, ty)], fill=(255, 255, 50, 255), width=4)

                    elif el_name == "ice":
                        tx = cx + (circle_r + 40) * math.cos(rad)
                        ty = cy + (circle_r + 40) * math.sin(rad)
                        lx = cx + (circle_r + 5) * math.cos(math.radians(ang - 4))
                        ly = cy + (circle_r + 5) * math.sin(math.radians(ang - 4))
                        rx = cx + (circle_r + 5) * math.cos(math.radians(ang + 4))
                        ry = cy + (circle_r + 5) * math.sin(math.radians(ang + 4))
                        draw.polygon([(lx, ly), (tx, ty), (rx, ry)], fill=(160, 230, 255, 220))
                        
                    elif el_name == "water":
                        drop_r = 10
                        draw.ellipse([bx-drop_r, by-drop_r, bx+drop_r, by+drop_r], fill=(0, 150, 255, 200))
                        tx = cx + (circle_r + 35) * math.cos(rad)
                        ty = cy + (circle_r + 35) * math.sin(rad)
                        lx = bx + drop_r * math.cos(rad - math.pi/2)
                        ly = by + drop_r * math.sin(rad - math.pi/2)
                        rx = bx + drop_r * math.cos(rad + math.pi/2)
                        ry = by + drop_r * math.sin(rad + math.pi/2)
                        draw.polygon([(lx, ly), (tx, ty), (rx, ry)], fill=(0, 150, 255, 200))

                    elif el_name == "plant":
                        next_rad = math.radians(ang + step_deg)
                        nx = cx + (circle_r + 10) * math.cos(next_rad)
                        ny = cy + (circle_r + 10) * math.sin(next_rad)
                        draw.line([(bx, by), (nx, ny)], fill=(34, 139, 34, 255), width=4)
                        
                        lx1 = cx + (circle_r + 20) * math.cos(math.radians(ang - 8))
                        ly1 = cy + (circle_r + 20) * math.sin(math.radians(ang - 8))
                        lx2 = cx + (circle_r + 20) * math.cos(math.radians(ang + 8))
                        ly2 = cy + (circle_r + 20) * math.sin(math.radians(ang + 8))
                        tx = cx + (circle_r + 35) * math.cos(rad)
                        ty = cy + (circle_r + 35) * math.sin(rad)
                        draw.polygon([(bx, by), (lx1, ly1), (tx, ty), (lx2, ly2)], fill=(40, 180, 40, 230))
                        
                    elif el_name == "rock":
                        pts = []
                        for offset in [(-10, -5), (0, -15), (12, -2), (8, 12), (-8, 10)]:
                            prad = math.atan2(offset[1], offset[0]) + rad + math.pi/2
                            dist = math.hypot(offset[0], offset[1])
                            pts.append((bx + dist * math.cos(prad), by + dist * math.sin(prad)))
                        draw.polygon(pts, fill=(120, 110, 110, 240), outline=(80, 70, 70, 255))

                    elif el_name == "holy":
                        tx = cx + (circle_r + 55) * math.cos(rad)
                        ty = cy + (circle_r + 55) * math.sin(rad)
                        w = 5 if (ang // step_deg) % 2 == 0 else 2
                        draw.line([(bx, by), (tx, ty)], fill=(255, 240, 120, 240), width=w)

                    elif el_name == "magic":
                        draw.ellipse([bx-8, by-8, bx+8, by+8], fill=(180, 50, 255, 220))
                        mx = cx + (circle_r + 30) * math.cos(math.radians(ang + 8))
                        my = cy + (circle_r + 30) * math.sin(math.radians(ang + 8))
                        draw.ellipse([mx-4, my-4, mx+4, my+4], fill=(255, 120, 255, 240))

                    elif el_name == "necro":
                        tx = cx + (circle_r + 35) * math.cos(rad)
                        ty = cy + (circle_r + 35) * math.sin(rad)
                        draw.line([(bx, by), (tx, ty)], fill=(220, 220, 220, 255), width=5)
                        nx1 = tx + 5 * math.cos(rad + math.pi/2)
                        ny1 = ty + 5 * math.sin(rad + math.pi/2)
                        draw.ellipse([nx1-4, ny1-4, nx1+4, ny1+4], fill=(220, 220, 220, 255))
                        nx2 = tx + 5 * math.cos(rad - math.pi/2)
                        ny2 = ty + 5 * math.sin(rad - math.pi/2)
                        draw.ellipse([nx2-4, ny2-4, nx2+4, ny2+4], fill=(220, 220, 220, 255))

                    elif el_name == "air":
                        pts = []
                        for step_a in range(0, 40, 10):
                            curve_rad = math.radians(ang + step_a)
                            dist = circle_r + 10 + step_a
                            pts.append((cx + dist * math.cos(curve_rad), cy + dist * math.sin(curve_rad)))
                        draw.line(pts, fill=(180, 240, 255, 230), width=4)

                    elif el_name == "psychic":
                        for roff in [10, 25, 40]:
                            arc_r = circle_r + roff
                            draw.arc([cx - arc_r, cy - arc_r, cx + arc_r, cy + arc_r], ang - 5, ang + 5, fill=(255, 50, 200, 220), width=4)

                    elif el_name == "fighting":
                        lx = cx + (circle_r + 5) * math.cos(math.radians(ang - 6))
                        ly = cy + (circle_r + 5) * math.sin(math.radians(ang - 6))
                        rx = cx + (circle_r + 5) * math.cos(math.radians(ang + 6))
                        ry = cy + (circle_r + 5) * math.sin(math.radians(ang + 6))
                        tx = cx + (circle_r + 28) * math.cos(rad)
                        ty = cy + (circle_r + 28) * math.sin(rad)
                        draw.polygon([(lx, ly), (tx, ty), (rx, ry)], fill=(220, 40, 40, 255), outline=(100, 0, 0, 255))

                    else: # basic (fallback for standard / unhandled elements)
                        draw.ellipse([bx-6, by-6, bx+6, by+6], fill=(255, 215, 0, 255))
                        tx = cx + (circle_r + 25) * math.cos(rad)
                        ty = cy + (circle_r + 25) * math.sin(rad)
                        lx = cx + (circle_r + 15) * math.cos(math.radians(ang - 6))
                        ly = cy + (circle_r + 15) * math.sin(math.radians(ang - 6))
                        rx = cx + (circle_r + 15) * math.cos(math.radians(ang + 6))
                        ry = cy + (circle_r + 15) * math.sin(math.radians(ang + 6))
                        hx = cx + (circle_r + 35) * math.cos(rad)
                        hy = cy + (circle_r + 35) * math.sin(rad)
                        draw.polygon([(tx, ty), (lx, ly), (hx, hy), (rx, ry)], fill=(218, 165, 32, 255))

            # Render logic based on whether they have Dual Elements or a Single Element
            if element1 and element2 and element1 != element2:
                # Top half (180 to 360 degrees) for Element 1
                draw_element_decorator(d, element1, 180, 360)
                # Bottom half (0 to 180 degrees) for Element 2
                draw_element_decorator(d, element2, 0, 180)
            else:
                # Full circle (0 to 360 degrees) for Single Element
                draw_element_decorator(d, element1, 0, 360)
