...
class GameSession:
    ...
    def get_participant_name(self, p: Any) -> str:
        return self.participant_names.get(p, str(p))
    ...
    def _species_emoji(self, participant: Any) -> str:
        try:
            ...
            if pet is None and not self._pets_loaded:
                udm = UserDataManager() if UserDataManager else None
                if udm:
                    pet = udm.get_pet_data(uid)
            
            species = (pet or {}).get("species") or (pet or {}).get("name") or ""
            return get_pet_emoji("Pets", species) or "🐾"
        except Exception:
            return "🐾"
    ...
    async def _get_member_pet(self, participant: Any) -> Dict[str, Any]:
        try:
            ...
            elem_emoji = get_pet_emoji("Elements", element) or "⚡"
            cat_emoji = get_pet_emoji("Pet Type", category) or "🌿"
            return {"category": category, "element": element, "species": species, "species_emoji": species_emoji, "element_emoji": elem_emoji, "category_emoji": cat_emoji, "pet": pet}
        except Exception:
            return {"category": "land", "element": "basic", "species": "Pet", "species_emoji": "", "element_emoji": get_pet_emoji("Elements", "basic") or "⚖️", "category_emoji": get_pet_emoji("Pet Type", "land") or "🌿", "pet": None}
    ...
    def _build_action_embed(self, participant: Any, pet_info: Dict[str, Any]) -> Embed:
    ...
    def _build_elim_embed(self, participant: Any, pet_info: Dict[str, Any]) -> Embed:
        elem = pet_info["element"]
    ...
    class ChoiceView(ui.View):
        def __init__(self, choices: List[Dict[str, Any]], timeout_sec: int = 300):
            ...
            for ch in choices:
                ...
                btn = ui.Button(label=str(label)[:80], style=style)
                async def handler(interaction: discord.Interaction, choice=ch):
                    self.result = choice
                    try:
    ...
    async def _send_user_event(self, bot: commands.Bot, participant: Any) -> Tuple[str, Optional[str]]:
        ...
        if not user_obj and hasattr(participant, "id") and bot:
            user_obj = bot.get_user(getattr(participant, "id", None))
        if not user_obj:
    ...
    async def _handle_deadly_event(self, bot, p, round_timeout) -> Dict[str, Any]:
        ...
        if not success:
            ...
            res['markers'].append({'x': px, 'y': py, 'pos': (px, py), 'style': style, 'is_elimination': True, 'emoji': '💀', 'pattern': False})
        else:
    ...
    async def _handle_action_event(self, bot, p, round_timeout) -> Dict[str, Any]:
        ...
        if success:
            res['text'] = f"{name} succeeds: {self._apply_placeholders(choice.get('success_text') or choice.get('text'), [name], loc_name)}"
            res['markers'].append({'x': px, 'y': py, 'pos': (px, py), 'style': loc_style, 'is_elimination': False, 'emoji': self._species_emoji(p)})
        else:
            res['text'] = f"{name} fails: {self._apply_placeholders(choice.get('failure_text') or choice.get('text'), [name], loc_name)}"
            res['markers'].append({'x': px, 'y': py, 'pos': (px, py), 'style': loc_style, 'is_elimination': False, 'emoji': self._species_emoji(p)})
        ...
        if all_defend:
            ...
            for gp in group:
                ...
                if gp in fallen_local:
                    res['markers'].append({'x': loc['x'], 'y': loc['y'], 'pos': (loc['x'], loc['y']), 'style': style, 'cluster': (gcx, gcy), 'is_elimination': True, 'emoji': '💀', 'pattern': True})
                else:
                    res['markers'].append({'x': loc['x'], 'y': loc['y'], 'pos': (loc['x'], loc['y']), 'style': style, 'cluster': (gcx, gcy), 'is_elimination': False, 'emoji': self._species_emoji(gp), 'pattern': True})
    ...
    def process_round(self) -> Dict[str, Any]:
            ...
            if len(self.alive) >= 6 and random.random() < 0.05:
                ...
                for participant in side_a + side_b:
                    if participant in self.alive:
                        ...
                        self.round_markers.append({'x': ex, 'y': ey, 'pos': (ex, ey), 'style': loc_style, 'cluster': (cx, cy), 'is_elimination': True, 'emoji': '💀'})
        ...
        while unassigned:
            ...
            if not is_elim:
                ...
                for idx, p in enumerate(side_b):
                    px, py = coords_b[idx]
                    self.round_markers.append({'x': px, 'y': py, 'pos': (px, py), 'style': loc_style, 'cluster': shared_cluster_coords_b, 'is_elimination': True, 'emoji': '💀', 'side_a': side_a, 'side_b': side_b, 'owner_side': 'B'})
...
class GameSetupView(ui.View):
    ...
    @ui.button(label="Start Game", style=ButtonStyle.green, emoji=discord.PartialEmoji(id=emoji_mod.id_for('Series')), custom_id="survivor:start")
    async def start_game(self, interaction: discord.Interaction, button: ui.Button):
        ...
        await interaction.response.send_message("🐾 **Pet Survivor Series Started!** 🐾", ephemeral=False)
        ...
        control_view = RoundControlView(self.game_session, interaction.client)
        interaction.client.add_view(control_view)
        await control_view.send_round(interaction.channel)

    @ui.button(label="Cancel Game", style=ButtonStyle.red, emoji="❌", custom_id="survivor:cancel")
...
class RoundControlView(ui.View):
    ...
    @ui.button(label="Next Round", style=ButtonStyle.primary, emoji="➡️", custom_id="survivor:next")
    async def next_round(self, interaction: discord.Interaction, button: ui.Button):
        ...
        try:
            if len(self.game_session.alive) <= 1:
                await self._end_game(interaction.channel)
                return
            await self.send_round(interaction.channel)
            if self.message:
                await self.message.edit(view=self)
        except Exception as e:
            # If anything goes wrong, try to send an error message
            try:
                await interaction.channel.send(f"Error advancing round: {str(e)}")
            except:
                pass

    @ui.button(label="End Games", style=ButtonStyle.red, emoji="🛑", custom_id="survivor:end")
    async def end_games(self, interaction: discord.Interaction, button: ui.Button):
        ...
        try:
            ...
            # If anything goes wrong, try to send an error message
            try:
                await interaction.channel.send(f"Error ending game: {str(e)}")
            except:
                pass
    ...
    async def send_round(self, channel: discord.abc.Messageable):
        ...
        try:
            if not self.game_session.state_path:
                gid = getattr(channel, 'guild', None).id if getattr(channel, 'guild', None) else 0
        ...
        try:
            ...
            if category == "air" and element == "fire":
                ...
                for i in range(10):
                    ang = 240 + i * 12
                    r = math.radians(ang)
                    bx = cx + (circle_r + 16) * math.cos(r)
                    by = cy + (circle_r + 16) * math.sin(r)
                    tx = cx + (circle_r + 44) * math.cos(r)
                    ty = cy + (circle_r + 44) * math.sin(r)
                    sx = cx + (circle_r + 28) * math.cos(r + 0.12)
                    sy = cy + (circle_r + 28) * math.sin(r + 0.12)
    ...
    async def _generate_champion_image(self, winner: str) -> Optional[BytesIO]:
        ...
        try:
            ...
            info = await self._get_member_pet(winner)
...
class PetSurvivorSeries(commands.Cog):
        ...
        try:
            ...
            if self.active_sessions:
                ...
                for (gid, cid), (gs, path) in list(self.active_sessions.items()):
                    try:
                        v = RoundControlView(gs, self.bot) if gs.started else GameSetupView(gs)
                        self.bot.add_view(v)
                    except Exception:
                        continue
        except Exception as e:
    ...
    async def series(self, ctx: commands.Context, warriors: int, users: str = "", bots: bool = False, roles: str = "", official: bool = False):
        ...
        embed = Embed(
            title=(f"{emoji_mod.mention('Series') or '🎲'} __{ord_text}__ OFFICIAL 🎟️ PET SURVIVOR SERIES" if official else f"{emoji_mod.mention('Series') or '🎲'} PET SURVIVOR SERIES"),
            description=(
                f"{prefix} "
        ...
        for p in participants:
            loc = game_session.locations.get(p, {})
    ...
    @survivor.command(name="champions")
    async def champions(self, ctx: commands.Context):
        ...
        class ChampionsView(ui.View):
            def __init__(self, pages: List[Embed]):
                ...
                self.index = 0
                self.message = None
            @ui.button(label="Previous", style=ButtonStyle.gray, emoji="◀️")
            async def prev(self, interaction: discord.Interaction, button: ui.Button):
                self.index = (self.index - 1) % len(self.pages)
                await interaction.response.edit_message(embed=self.pages[self.index], view=self)
            @ui.button(label="Next", style=ButtonStyle.gray, emoji="▶️")
