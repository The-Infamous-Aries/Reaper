...
class TournamentBattleView(discord.ui.View):
    ...
    def create_hp_bar(self, current_hp: int, max_hp: int, pet: dict = None) -> str:
        ...
        empty_char = '⬛'
        bar = filled_bar + empty_char * (bar_length - filled_length)
    ...
    async def process_attack(self, attacker_id: str, target_id: str):
        ...
        self.battle_log.append(
            f"⚔️ {attacker['user'].display_name} uses {atk_verb} on {target['user'].display_name} for {final_damage} damage!"
        )
    ...
    async def charge_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            labels = DamageCalculator.get_action_labels(str(pet.get('category','')).lower(), pelem, species=pet.get('species'))
            elem_emojis = {
    ...
    async def defend_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            labels = DamageCalculator.get_action_labels(ptype, str(pet.get('element','')).lower(), species=pet.get('species'))
                ...
                if self.current_round >= self.max_rounds:
                    ...
                    if champion:
                        try:
                            ...
                            leveled_up, details = await add_experience(str(champion.id), tournament_bonus_xp, "tournament", None)
                            if leveled_up:
                                await send_level_up_embed(str(champion.id), details, self.channel)
                        except Exception as e:
