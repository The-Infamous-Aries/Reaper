...
class BettingView(discord.ui.View):
    def __init__(self, session: 'HoldemSession', player: PlayerState, call_amount: int):
        ...
        if call_amount == 0:
            self.btn_call.label = "Check"
            self.btn_call.style = discord.ButtonStyle.secondary
        else:
            self.btn_call.label = f"Call ({call_amount})"
            self.btn_call.style = discord.ButtonStyle.primary

    @discord.ui.button(label="Bet/Raise", style=discord.ButtonStyle.success)
...
class HoldemSession(discord.ui.View):
    ...
    def _build_table_embed(self) -> discord.Embed:
        e = discord.Embed(title="Texas Hold'em", color=discord.Color.dark_red(), timestamp=discord.utils.utcnow())
