...
class BlackjackSession(discord.ui.View):
    def __init__(self, bot: commands.Bot, channel_id: int, solo: bool, betting_mode: bool, buy_in: int, host: discord.Member, bot_count: int = 0):
        ...
        for i in range(bot_count):
            bot_id = -(i + 1)
            bot_member = BotMember(bot_id, f"Bot {i+1}", f"Bot {i+1}")
    ...
    def _build_bets_embed(self) -> discord.Embed:
        e = discord.Embed(title="Blackjack Bets", color=discord.Color.blurple(), timestamp=discord.utils.utcnow())
    ...
    def _build_table_embed(self) -> discord.Embed:
        e = discord.Embed(title="Blackjack", color=discord.Color.dark_green(), timestamp=discord.utils.utcnow())
        ...
        for uid in self.current_player_order:
            ...
            for i, hand in enumerate(ps.hands):
                ...
                turn_mark = "▶" if uid == self._current_player_id() and i == ps.active_hand_index else ""
