...
class LootCalculator:
    ...
    def group_inventory_items(inventory: List[Dict[str, Any]]) -> Tuple[Dict[str, Dict[str, int]], Dict[str, Dict[str, Any]]]:
        ...
        item_counts = {} # type -> name -> count
        item_data = {} # type -> name -> item_obj
    ...
    @staticmethod
    def _consolidate_inventory(inventory: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Consolidates duplicate items into single items with a 'count' field.
        Used to migrate old inventories or clean up data.
        """
        consolidated_map = {} # key -> item
        ...
        for item in inventory:
            ...
            if key not in consolidated_map:
                ...
                if "count" not in new_item:
                    new_item["count"] = 1
                consolidated_map[key] = new_item
            else:
                # Duplicate found, increment count
                existing_count = consolidated_map[key].get("count", 1)
                add_count = item.get("count", 1)
                consolidated_map[key]["count"] = existing_count + add_count
                
        return list(consolidated_map.values())
    ...
    def get_pet_emoji(category: str, key: Union[str, int]) -> str:
        ...
        return emoji_mod.mention(target_key) or emoji_mod.get(target_key) or ""

    @staticmethod
    def get_pet_stat_emojis() -> Dict[str, str]:
        stats = ["ATT", "DEF", "INT", "DEX", "HAP", "ENE"]
        defaults = {"ATT": "⚔️", "DEF": "🛡️", "INT": "🧠", "DEX": "🦶", "HAP": "😊", "ENE": "⚡"}
        return {s: LootCalculator.get_pet_emoji("Stats", s) or defaults.get(s, "") for s in stats}

    @staticmethod
    def get_pet_source_emojis() -> Dict[str, str]:
        return {
            "battle": LootCalculator.get_pet_emoji("Pet Helpers", "ring") or '⚔️',
            "train": LootCalculator.get_pet_emoji("Pet Helpers", "Train") or '🏋️',
            "training": LootCalculator.get_pet_emoji("Pet Helpers", "Train") or '🏋️',
            ...
            "pvp": LootCalculator.get_pet_emoji("Pet Helpers", "PvP") or '⚔️',
            "series": LootCalculator.get_pet_emoji("Pet Helpers", "Series") or '🏆',
            "tournament": LootCalculator.get_pet_emoji("Pet Helpers", "Tournament") or '🏟️',
            "gamble": '🎲'
        }
    ...
    async def create_level_up_embed(pet_data: dict, old_level: int, new_level: int, source: str = "battle") -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"{src_emoji} LEVEL UP! {src_emoji}",
    ...
    async def create_level_down_embed(pet_data: dict, old_level: int, new_level: int, source: str = "mission", lost_xp: int = 0) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"{src_emoji} LEVEL DOWN {src_emoji}",
    ...
    @staticmethod
    async def add_item_to_inventory(user_id: int, item: Dict[str, Any], pet_data: Dict[str, Any] = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            inventory = LootCalculator._consolidate_inventory(inventory)
        ...
        try:
            ...
            if etype == "elemental_boost":
                ...
                try:
                    ...
                    # Apply XP Change properly (handles level ups)
                    current_xp = int(pet.get("experience", 0))
                    base_xp = LootCalculator.get_total_experience_for_level(level)
                    current_total = base_xp + current_xp
                    new_total = current_total + xp_gain
                    
                    _, new_level, new_xp = LootCalculator.recompute_level_from_total_xp(pet, new_total)
                    
                    pet["level"] = new_level
                    pet["experience"] = new_xp
                    
                    changes.append(f"+{xp_gain} XP")
                    if new_level > level:
                         changes.append(f"**LEVEL UP!** {level} -> {new_level}")
                         # Add level up stats
                         lvl_gains = LootCalculator.calculate_level_up_stats(pet, level, new_level)
                         for k, v in lvl_gains.items():
    ...
    @staticmethod
    async def equip_items(user_id: str, material_name: Optional[str] = None, gem_names: Optional[str] = None, monster_names: Optional[str] = None, hat_name: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Helper to find and remove item (decrement count)
            def take_item(name, cat_key):
                for i, item in enumerate(inventory):
                    itype = item.get('type', 'Material')
                    # Check name and type match
                    if item.get('name') == name.strip() and cat_key in itype:
                        count = item.get('count', 1)
                        if count > 0:
                            item['count'] = count - 1
                            # Create a clean copy for equipment (count=1 effectively)
                            equipped_item = item.copy()
                            if 'count' in equipped_item:
                                del equipped_item['count']
                            
                            # If count drops to 0, remove from inventory
                            if item['count'] <= 0:
                                inventory.pop(i)
            ...
            # 1. Material
            if material_name:
                new_mat = take_item(material_name, "Material")
                if new_mat:
                    # Unequip old
                    old_mat = equipment.get("Material")
                    if old_mat:
                        _, msg = LootCalculator._return_item_to_inventory(inventory, old_mat, pet, int(user_id))
                        if msg: msg_parts.append(msg)
                    equipment["Material"] = new_mat
                    emoji = LootCalculator.get_pet_emoji("Material", material_name) or "🪵"
                    msg_parts.append(f"{emoji} Equipped **{material_name}**")
                else:
                    msg_parts.append(f"❌ Could not find material **{material_name}**")

            # 2. Hat
            if hat_name:
                new_hat = take_item(hat_name, "Hat")
                if new_hat:
                    # Unequip old
                    old_hat = equipment.get("Hat")
                    if old_hat:
                        _, msg = LootCalculator._return_item_to_inventory(inventory, old_hat, pet, int(user_id))
                        if msg: msg_parts.append(msg)
                    equipment["Hat"] = new_hat
                    emoji = LootCalculator.get_pet_emoji("Hats", hat_name) or "🧢"
                    msg_parts.append(f"{emoji} Equipped **{hat_name}**")
                else:
                    msg_parts.append(f"❌ Could not find hat **{hat_name}**")

            # 3. Gems
            if gem_names:
                names = [n.strip() for n in gem_names.split(',') if n.strip()]
                new_gems = []
                missing = []
                # Try to take up to 2
                for n in names[:2]: 
                    found = take_item(n, "Gem")
                    if found:
                        new_gems.append(found)
                    else:
                        missing.append(n)
                
                if new_gems:
                    # Unequip all old gems
                    old_gems = equipment.get("Gems", [])
                    if isinstance(old_gems, list):
                        for g in old_gems: 
                            _, msg = LootCalculator._return_item_to_inventory(inventory, g, pet, int(user_id))
                            if msg: msg_parts.append(msg)
                    elif isinstance(old_gems, dict): 
                         _, msg = LootCalculator._return_item_to_inventory(inventory, old_gems, pet, int(user_id))
                         if msg: msg_parts.append(msg)
                    
                    equipment["Gems"] = new_gems
                    gem_names_list = [g['name'] for g in new_gems]
                    emoji = "💎"
                    msg_parts.append(f"{emoji} Equipped Gems: **{', '.join(gem_names_list)}**")
                
                if missing:
                    msg_parts.append(f"❌ Missing Gems: {', '.join(missing)}")

            # 3. Monsters
            if monster_names:
                names = [n.strip() for n in monster_names.split(',') if n.strip()]
                new_mons = []
                missing = []
                for n in names[:2]:
                    found = take_item(n, "Monster")
                    if found:
                        new_mons.append(found)
                    else:
                        missing.append(n)
                
                if new_mons:
                    old_mons = equipment.get("Monsters", [])
                    if isinstance(old_mons, list):
                        for m in old_mons: 
                            _, msg = LootCalculator._return_item_to_inventory(inventory, m, pet, int(user_id))
                            if msg: msg_parts.append(msg)
                    elif isinstance(old_mons, dict):
                        _, msg = LootCalculator._return_item_to_inventory(inventory, old_mons, pet, int(user_id))
                        if msg: msg_parts.append(msg)
                    
                    equipment["Monsters"] = new_mons
                    mon_names_list = [m['name'] for m in new_mons]
                    emoji = "👹"
                    msg_parts.append(f"{emoji} Equipped Monsters: **{', '.join(mon_names_list)}**")

                if missing:
                    msg_parts.append(f"❌ Missing Monsters: {', '.join(missing)}")
        ...
        try:
            ...
            if slot == "Material":
                ...
                if isinstance(items, list):
                    for i in items: 
                        _, msg = LootCalculator._return_item_to_inventory(inventory, i, pet, int(user_id))
                        if msg: xp_msgs.append(msg)
                    items_removed.extend([i.get("name", "Unknown Gem") for i in items])
                elif isinstance(items, dict):
                    _, msg = LootCalculator._return_item_to_inventory(inventory, items, pet, int(user_id))
                    if msg: xp_msgs.append(msg)
                    items_removed.append(items.get("name", "Unknown Gem"))
                if "Gems" in equipment:
                    del equipment["Gems"]
                ...
                if isinstance(items, list):
                    for i in items: 
                        _, msg = LootCalculator._return_item_to_inventory(inventory, i, pet, int(user_id))
                        if msg: xp_msgs.append(msg)
                    items_removed.extend([i.get("name", "Unknown Monster") for i in items])
                elif isinstance(items, dict):
                    _, msg = LootCalculator._return_item_to_inventory(inventory, items, pet, int(user_id))
                    if msg: xp_msgs.append(msg)
                    items_removed.append(items.get("name", "Unknown Monster"))
                if "Monsters" in equipment:
                    del equipment["Monsters"]
    ...
    @staticmethod
    def get_key_loot(difficulty: str = "normal") -> Optional[Dict[str, Any]]:
        # Rare drop: 5% Easy, 10% Medium, 15% Hard
        chance = 0.05
        diff = difficulty.lower()
        if diff == "medium": chance = 0.10
        elif diff == "hard": chance = 0.15
        ...
        keys = ["Key1", "Key2", "Key3"]
        # Weight towards Key1
        name = random.choices(keys, weights=[60, 30, 10], k=1)[0]
    ...
    @staticmethod
    async def open_chest(user_id: int, chest_type: str, amount: int, selected_type: str = None) -> List[str]:
        """
    ...
    async def apply_xp_change(user_id: int, xp_amount: int, source: str = "unknown") -> Tuple[bool, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            pet_data[xp_key] = int(pet_data.get(xp_key, 0)) + int(xp_amount)
            if xp_amount > 0:
            ...
            if has_changed:
                if new_level > old_level:
                    ...
                    # Level Down (optional logic if needed)
                    pass
            
            await user_data_manager.save_pet_data(str(user_id), pet_data.get("name", "Pet"), pet_data)
            return has_changed, change_data
    ...
    async def calculate_loot(
        ...
        if is_winner:
            ...
            loot_result = await LootCalculator.award_loot_items(user_id, pet_data, difficulty, pet_data.get("level", 1))
    ...
    @staticmethod
    async def award_loot_items(user_id: int, pet_data: Dict[str, Any], difficulty: str = "normal", level: int = 1) -> Dict[str, Any]:
        """
        Awards all potential loot items based on difficulty.
        Returns dict with "items_gained" (list) and "messages" (list).
        ONLY Keys are awarded now.
        """
        result = {"items_gained": [], "messages": []}
        items_to_add = []
        
        # Keys
        key = LootCalculator.get_key_loot(difficulty)
        if key: items_to_add.append(key)

        # Materials
        material = LootCalculator.get_material_loot_item(difficulty, level)
        if material: items_to_add.append(material)

        # Gems
        gem = LootCalculator.get_gem_loot_item(difficulty)
        if gem: items_to_add.append(gem)

        # Monsters
        monster = LootCalculator.get_monster_loot_item(difficulty)
        if monster: items_to_add.append(monster)

        # Potions
        potion = LootCalculator.get_potion_loot(difficulty)
        if potion: items_to_add.append(potion)

        # Hats
        hat = LootCalculator.get_hat_loot_item(difficulty)
        if hat: items_to_add.append(hat)
...
class StatsCalculator:
    ...
    def calculate_pet_stats(pet_data: Dict[str, Any]) -> Dict[str, int]:
        ...
        Logic:
        1. Start with base stats (ATT, DEF, INT, DEX, HAP, ENE).
        2. Add bonuses from equipment (Material, Gems, Monsters).
        3. Specialization Bonus: If 2 identical Gems or Monsters are equipped and they boost a Spec skill, 
           the bonus for that skill is TRIPLED.
        4. Level Milestone Bonus: Every 50 levels, ALL equipment bonuses are multiplied by (1 + level // 50).
        """
        # Base stats
        stats = {
        ...
        # Level Milestone Bonus: x2 at 50, x3 at 100, etc.
        # Formula: 1 + (level // 50)
        ...
        if hat and isinstance(hat, dict):
        ...
        # Identify duplicates for Specialization Bonus
        # "If a user has two of the same Monsters or Gems equiped"
        gem_counts = {}
        mon_counts = {}
        ...
        equipment_bonuses = {k: 0 for k in stats}
        
        for type_key, item in items:
            bonuses = item.get('bonuses', {})
            name = item.get('name')
            
            # Check for duplicate pair condition
            is_duplicate_pair = False
            if type_key == 'Gem' and name and gem_counts.get(name, 0) >= 2:
            ...
            for stat, val in bonuses.items():
                ...
                mult = 1
                # Specialization Bonus: Triple bonus if duplicate pair AND stat is in specs
                if is_duplicate_pair and stat in specs:
                    mult = 3
                
                equipment_bonuses[stat] += val * mult
                
        # Apply Level Milestone Bonus to total equipment bonuses and add to base
        for stat in stats:
            bonus = equipment_bonuses.get(stat, 0) * level_mult
            stats[stat] += bonus
...
class DamageCalculator:
    ...
    def calculate_battle_action(
        ...
        element_bonus = DamageCalculator.compute_element_bonus(attacker_element, defender_element)
        ...
        if target_action_type == "defend":
            ...
            def_element_bonus = DamageCalculator.compute_element_bonus(defender_element, attacker_element)
    ...
    @staticmethod
    def calculate_pet_health(hap: int, ene: int, level: int = 1) -> int:
        try:
            hap_i = max(0, int(hap))
            ene_i = max(0, int(ene))
            lvl_i = max(1, int(level))
        except Exception:
            hap_i, ene_i, lvl_i = 0, 0, 1
        return hap_i * ene_i * lvl_i * 50
    ...
    @staticmethod
    def get_action_labels(pet_type: str, pet_element: str, species: str = None) -> Dict[str, str]:
        ...
        labels: Dict[str, Dict[str, Dict[str, str]]] = {
            "flying": {
                "basic": {"attack": "Wing Strike", "defend": "Feather Guard", "charge": "Focus Air"},
                "fire": {"attack": "Dive Scratch", "defend": "Barrel Roll", "charge": "Smolder"},
                "water": {"attack": "Splash Peck", "defend": "Mist Veil", "charge": "Cloud Surge"},
                "electric": {"attack": "Thunder Dive", "defend": "Static Slip", "charge": "Sky Charge"},
                "ice": {"attack": "Frost Talon", "defend": "Glacier Glide", "charge": "Cryo Draft"},
                "plant": {"attack": "Leaf Gust", "defend": "Vine Wing Shield", "charge": "Bramble Focus"},
                "rock": {"attack": "Stone Swoop", "defend": "Cliff Drift", "charge": "Stone Momentum"},
                "air": {"attack": "Gale Slash", "defend": "Wind Wall", "charge": "Zephyr Coil"},
                "magic": {"attack": "Arcane Dive", "defend": "Rune Wing", "charge": "Aether Spin"},
                "holy": {"attack": "Radiant Talon", "defend": "Halo Drift", "charge": "Seraph Breath"},
                "necro": {"attack": "Ghoul Peck", "defend": "Shade Veil", "charge": "Grave Gale"},
            },
            "land": {
                "basic": {"attack": "Tackle", "defend": "Guard Stance", "charge": "Focus Ground"},
                "fire": {"attack": "Ember Pounce", "defend": "Cinder Guard", "charge": "Ignite Core"},
                "water": {"attack": "Mud Slam", "defend": "Ripple Brace", "charge": "Tide Focus"},
                "electric": {"attack": "Volt Dash", "defend": "Static Brace", "charge": "Amp Charge"},
                "ice": {"attack": "Frost Kick", "defend": "Glacial Brace", "charge": "Crystal Focus"},
                "plant": {"attack": "Thorn Swipe", "defend": "Root Brace", "charge": "Bloom Focus"},
                "rock": {"attack": "Boulder Bash", "defend": "Granite Guard", "charge": "Ore Charge"},
                "air": {"attack": "Dust Rush", "defend": "Dust Screen", "charge": "Wind Up"},
                "magic": {"attack": "Hex Strike", "defend": "Ward Stance", "charge": "Mana Coil"},
                "holy": {"attack": "Smite Slam", "defend": "Sanctum Brace", "charge": "Faith Chant"},
                "necro": {"attack": "Bone Claw", "defend": "Crypt Shroud", "charge": "Grave Bind"},
            },
            "swimming": {
                "basic": {"attack": "Fin Strike", "defend": "Shell Guard", "charge": "Focus Tide"},
                "fire": {"attack": "Steam Bite", "defend": "Vapor Veil", "charge": "Boil Focus"},
                "water": {"attack": "Torrent Lunge", "defend": "Wave Wall", "charge": "Deep Coil"},
                "electric": {"attack": "Shock Snap", "defend": "Current Slip", "charge": "Surge Charge"},
                "ice": {"attack": "Icebite", "defend": "Ice Veil", "charge": "Glacier Coil"},
                "plant": {"attack": "Reed Whip", "defend": "Kelp Screen", "charge": "Bloom Current"},
                "rock": {"attack": "Pebble Rake", "defend": "Coral Brace", "charge": "Gravel Coil"},
                "air": {"attack": "Foam Burst", "defend": "Spray Screen", "charge": "Mist Coil"},
                "magic": {"attack": "Tide Hex", "defend": "Rune Foam", "charge": "Abyss Chant"},
                "holy": {"attack": "Blessed Surge", "defend": "Sanct Tide", "charge": "Sacred Flow"},
                "necro": {"attack": "Dread Current", "defend": "Mire Veil", "charge": "Dark Undertow"},
            },
        }
        type_map = labels.get(t, labels["land"])
        entry = type_map.get(e, type_map["basic"])
        return {"attack": entry["attack"], "defend": entry["defend"], "charge": entry["charge"]}
    
    @staticmethod
    def compute_type_bonus(attacker_type: str, defender_type: str) -> float:
        try:
    ...
    @staticmethod
    def compute_element_bonus(attacker_element: str, defender_element: str) -> float:
        try:
            a_e = str(attacker_element or "").lower()
            d_e = str(defender_element or "").lower()
            return DamageCalculator.ELEMENT_EFFECTIVENESS.get(a_e, {}).get(d_e, 1.0)
        except Exception:
            return 1.0
