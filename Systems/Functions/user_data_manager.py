...
class UserDataManager:
    ...
    async def _flush_user_internal(self, user_id: str):
        ...
        await self.file_manager.save_async(path, data)
    ...
    async def update_pet_gambling_stats(self, user_id: str, game_type: str, winnings: int, bet_amount: int = 0, extra_data: Optional[Dict[str, Any]] = None) -> bool:
        ...
        async with self._user_lock(user_id):
            ...
            # Standard stats
            if game_type == "races":
                 game_stats["races_played"] = int(game_stats.get("races_played", 0)) + 1
                 if winnings > 0:
                     game_stats["races_won"] = int(game_stats.get("races_won", 0)) + 1
                 else:
                     game_stats["races_lost"] = int(game_stats.get("races_lost", 0)) + 1
            elif game_type == "slots":
                 game_stats["total_games_played"] = int(game_stats.get("total_games_played", 0)) + 1
            elif game_type == "blackjack":
                 game_stats["rounds_played"] = int(game_stats.get("rounds_played", 0)) + 1
                 if winnings > 0:
                     game_stats["rounds_won"] = int(game_stats.get("rounds_won", 0)) + 1
                 elif winnings < 0:
                     game_stats["rounds_lost"] = int(game_stats.get("rounds_lost", 0)) + 1
            else: # holdem, craps
                 game_stats["games_played"] = int(game_stats.get("games_played", 0)) + 1
                 if winnings > 0:
                     game_stats["games_won"] = int(game_stats.get("games_won", 0)) + 1
                 elif winnings < 0:
                     game_stats["games_lost"] = int(game_stats.get("games_lost", 0)) + 1

            # XP Stats
            if winnings > 0:
                game_stats["xp_won_total"] = int(game_stats.get("xp_won_total", 0)) + int(winnings)
                game_stats["highest_xp_win"] = max(int(game_stats.get("highest_xp_win", 0)), int(winnings))
            else:
                game_stats["xp_lost_total"] = int(game_stats.get("xp_lost_total", 0)) + int(abs(winnings))

            if bet_amount > 0:
                game_stats["highest_xp_bet"] = max(int(game_stats.get("highest_xp_bet", 0)), int(bet_amount))

            # Extra Data (Game Specific)
            if extra_data:
                for key, value in extra_data.items():
                    if isinstance(value, dict):
                        # Nested dict merge (e.g. games_by_difficulty)
                        target = game_stats.setdefault(key, {})
                        for sub_k, sub_v in value.items():
                             target[sub_k] = int(target.get(sub_k, 0)) + int(sub_v)
                    else:
                        # Direct increment/update
                        current = int(game_stats.get(key, 0))
                        game_stats[key] = current + int(value)
    ...
    def _consolidate_inventory(self, inventory: List[Any]) -> List[Dict[str, Any]]:
        """
        ...
        async with self._user_lock(user_id):
            ...
            for existing in inventory:
                if (existing.get("name") == name and 
                    existing.get("type") == itype and 
                    existing.get("rarity") == rarity):
                    
                    existing["count"] = existing.get("count", 1) + count_to_add
                    found = True
                    break
    ...
    async def increment_pet_stats(self, user_id: str, increments: Dict[str, Union[int, float]]) -> bool:
        """
        Increment numeric statistics for a pet safely.
        """
        async with self._user_lock(user_id):
            data = await self._get_user_data_internal(user_id)
            if not data or "pets" not in data or "pet_data" not in data["pets"]:
