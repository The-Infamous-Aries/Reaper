...
PERSISTENT_VIEWS_FILE = os.path.join(os.path.dirname(__file__), "treaties_views.json")
...
PillowImageFontModule = Optional[ModuleType]
...
ImageFont: PillowImageFontModule = None

try:
    import aiohttp as _aiohttp
    import io as _io
    from PIL import Image as _Image, ImageOps as _ImageOps, ImageDraw as _ImageDraw, ImageFont as _ImageFont
    aiohttp = _aiohttp
    io = _io
    Image = _Image
    ImageOps = _ImageOps
    ...
    # Dependencies not available, will remain None
    pass
...
class AuditManager(commands.Cog):
        ...
        try:
            ...
            if not cur:
                if self.default_alliance_id:
                    try:
                        choices.append(app_commands.Choice(name=f"{self.default_alliance_name} (ID {self.default_alliance_id})", value=str(int(self.default_alliance_id))))
                    except Exception:
                        choices.append(app_commands.Choice(name=f"{self.default_alliance_name} (ID {self.default_alliance_id})", value=str(self.default_alliance_id)))
    ...
    async def _fetch_flag_image(self, url: str) -> Optional[Image.Image]:
        ...
        try:
            ...
            if url.startswith('/'):
                url = f"https://politicsandwar.com{url}"
    ...
    def _draw_quadratic_bezier(self, draw, p0, p1, p2, fill, width):
        """Draw a quadratic Bézier curve by approximating it with a series of short lines."""
        points = []
        steps = 20  # Number of line segments to approximate the curve
    ...
    def _generate_treaty_web_image_sync(
        ...
        try:
            ...
            COLOR_MAP: Dict[str, tuple] = {
                'Protectorate': (255, 0, 0, 200),  # Red
                'Extension':    (255, 0, 0, 200),  # Red
                'MDoAP':        (0, 0, 255, 200),   # Blue
                'MDP':          (0, 0, 255, 200),   # Blue
                'ODoAP':        (255, 255, 0, 200), # Yellow
                'ODP':          (255, 255, 0, 200), # Yellow
                'PIAT':         (0, 255, 0, 200),   # Green
                'NAP':          (0, 255, 0, 200),   # Green
            }
            ...
            # Draw lines between partner alliances that share treaties
            partner_ids_set = set(partner_positions.keys())  # Get all partner alliance IDs
            inter_alliance_lines_drawn = 0  # Debug counter
            
            # Debug: Log what we're filtering
            self.logger.info(f"Partner IDs in web: {list(partner_ids_set)}")
            self.logger.info(f"Center ID: {center_id}")
            
            for treaty in all_treaties:
                a1_id_raw = treaty.get('alliance1_id')
                a2_id_raw = treaty.get('alliance2_id')
                
                # Skip if either alliance is missing
                ...
                # Skip if this treaty involves the center alliance
                if a1_id == center_id or a2_id == center_id:
                    continue
                    
                # Only draw if both alliances are in our partner list
                if a1_id in partner_ids_set and a2_id in partner_ids_set and a1_id != a2_id:
                    ...
                    # Get the center of each alliance's flag
                    # We need a reliable way to get the image size. Let's find one of the placed items.
                    sample_img_obj = next((item.get('img') for item in all_placed_items if item.get('img')), None)
                    img_width = sample_img_obj.width if sample_img_obj else 48
                    img_height = sample_img_obj.height if sample_img_obj else 48
                    
                    center1 = (pos1[0] + img_width // 2, pos1[1] + img_height // 2)
                    center2 = (pos2[0] + img_width // 2, pos2[1] + img_height // 2)
                    
                    # Calculate control point for the curve
                    midpoint_x = (center1[0] + center2[0]) / 2
                    midpoint_y = (center1[1] + center2[1]) / 2

                    # Vector from canvas center to midpoint
                    cm_vec_x = midpoint_x - CENTER_X
                    cm_vec_y = midpoint_y - CENTER_Y

                    # Perpendicular vector to push the control point outwards
                    perp_vec_x = -cm_vec_y
                    perp_vec_y = cm_vec_x

                    # Normalize the perpendicular vector
                    perp_dist = math.sqrt(perp_vec_x**2 + perp_vec_y**2)
                    if perp_dist == 0:
                        perp_norm_x, perp_norm_y = 0, 0
                    else:
                        perp_norm_x = perp_vec_x / perp_dist
                        perp_norm_y = perp_vec_y / perp_dist

                    # Curve intensity proportional to the line's distance from the center
                    line_length = math.sqrt((center1[0] - center2[0])**2 + (center1[1] - center2[1])**2)
                    curve_intensity = line_length * 0.15  # Adjust this factor to change curvature
                    
                    control_point_x = midpoint_x + perp_norm_x * curve_intensity
                    control_point_y = midpoint_y + perp_norm_y * curve_intensity
                    control_point = (control_point_x, control_point_y)

                    line_type = self._normalize_treaty_type(treaty.get('treaty_type', ''))
                    self._draw_quadratic_bezier(draw, center1, control_point, center2, fill=pick_line_color(line_type), width=LINE_WIDTH)
                    inter_alliance_lines_drawn += 1
                else:
                    # Debug: Log filtered treaties
                    if a1_id != center_id and a2_id != center_id:  # Only log non-center treaties
                        self.logger.debug(f"Filtered treaty: {a1_id} -> {a2_id}, in partners: {a1_id in partner_ids_set}, {a2_id in partner_ids_set}")
            
            # Debug logging
            if all_treaties:
                self.logger.info(f"Drawing treaty web: {len(all_treaties)} total treaties, {inter_alliance_lines_drawn} inter-alliance lines drawn, {len(partner_ids_set)} partner alliances")

            for it in all_placed_items:
    ...
    async def _compose_treaty_web_image(self, treaties: List[Dict[str, Any]], center_alliance_id: Optional[int] = None) -> Optional[discord.File]:
        ...
        # Fetch all partner treaties in a single batched query
        all_treaties: List[Dict[str, Any]] = []
        ...
        if partner_ids:
            try:
                ...
                self.logger.error(f"Error fetching treaties for partners {partner_ids}: {e}")
        ...
        return await asyncio.to_thread(
            ...
            cy_img,
            all_treaties,
            center_id,  # Pass center_id to the sync function
            80  # CENTER_SIZE
        )
    ...
    def _format_treaties_embed_sync(self, treaties: List[Dict[str, Any]], center_alliance_id: Optional[int] = None, center_name: Optional[str] = None) -> discord.Embed:
        ...
        title = "💀 DB4D Treaties 📜" if center_id == int(self.default_alliance_id or 0) else f"{(center_name or 'Alliance').strip()} Treaties 📜"
    ...
    async def _audit_mmr_sync(self, active_members: List[Dict[str, Any]], mmr_mode: str) -> discord.Embed:
        if (mmr_mode or "basic") == "max":
            ...
            title = f"{emoji_mod.mention('Manuf') or '⚙️'} MMR Build Audit — Max"
            note = "Shows ALL nations below 5/5/5/3 per-city average."
        else:
            thresh = {"barracks": 0.0, "factory": 2.0, "air": 5.0, "drydock": 1.0}
            title = f"{emoji_mod.mention('Manuf') or '⚙️'} MMR Build Audit — Basic"
        ...
        if total_offenders == 0:
            ...
            category_embeds["All Clear"] = no_offenders_embed
        else:
            categories = {
                "50%+ off": (bucket_50_plus, "🟥"),
                "25–49% off": (bucket_25_49, "🟧"),
                "10–24% off": (bucket_10_24, "🟨"),
                "0–9% off": (bucket_0_9, "🟩"),
            }
            ...
            for name, (bucket, emoji) in categories.items():
                ...
                if bucket:
                    await self._add_category_fields(current_embed, name, emoji, bucket, suffix_builder=suffix_builder)
                else:
                    current_embed.add_field(name=f"{emoji} {name}", value="No offenders in this category.", inline=False)
                current_embed.set_footer(text=f"Active members checked: {len(active_members)} • Offenders in this category: {len(bucket)}")
                category_embeds[name] = current_embed
    ...
    async def audit_command(self, ctx: commands.Context, view: Literal["resources", "inactives", "color", "mmr"], alliance: Optional[str] = None, mmr_mode: Optional[Literal["basic", "max"]] = "basic"):
        ...
        try:
            ...
            if view == "mmr":
                try:
                    # This will now return a dictionary of embeds
                    category_embeds = await self._audit_mmr_sync(active_members, mmr_mode or "basic")

                    # Determine the initial category to display
                    initial_category = "50%+ off"
                    if "All Clear" in category_embeds:
                        initial_category = "All Clear"
                    elif "25–49% off" in category_embeds and category_embeds["25–49% off"].fields:
                        initial_category = "25–49% off"
                    elif "10–24% off" in category_embeds and category_embeds["10–24% off"].fields:
                        initial_category = "10–24% off"
                    elif "0–9% off" in category_embeds and category_embeds["0–9% off"].fields:
                        initial_category = "0–9% off"

                    initial_embed = category_embeds.get(initial_category, list(category_embeds.values())[0])
...
class TreatiesRefreshView(discord.ui.View):
    def __init__(self, cog: AuditManager, alliance_id: int, timeout: Optional[float] = 300.0):
        super().__init__(timeout=timeout)
        self.cog = cog
    ...
    async def refresh(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            embed = await asyncio.to_thread(self.cog._format_treaties_embed_sync, treaties, center_alliance_id=int(self.alliance_id))
            files: List[discord.File] = []
            if treaty_file:
                embed.set_image(url=f"attachment://{treaty_file.filename}")
                files = [treaty_file]
    ...
    async def on_timeout(self):
        ...
        if self.message:
            try:
                # Remove the view from the persistent storage
                ...
                # Optionally disable the button or remove the view from the message
                # For now, just log and let the message remain without an active view
            except Exception as e:
...
async def setup(bot: commands.Bot):
    ...
    if alliance_cog:
        ...
        if query_instance and calc_instance:
            ...
            for alliance_id in audit.persistent_views.values():
                try:
                    bot.add_view(TreatiesRefreshView(audit, alliance_id, timeout=None))
                except Exception as e:
                    audit.logger.error(f"Error adding persistent view for alliance {alliance_id}: {e}")