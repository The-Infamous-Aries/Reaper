...
class ResourceStocks(commands.Cog):
    ...
    def _get_next_game_turn_time(self) -> datetime:
        ...
        # Game turns happen at :15 minutes past every even hour (00:15, 02:15, 04:15, etc.)
        current_hour = now.hour
        current_minute = now.minute
        
        # Find next even hour
        if current_hour % 2 == 0:
            # We're in an even hour
            if current_minute < 15:
                # Next turn is at :15 of current hour
                next_turn = now.replace(minute=15, second=0, microsecond=0)
            else:
                # Next turn is at :15 of next even hour
                next_hour = current_hour + 2
                if next_hour >= 24:
                    next_hour = 0
                    next_day = now + timedelta(days=1)
                    next_turn = next_day.replace(hour=next_hour, minute=15, second=0, microsecond=0)
                else:
                    next_turn = now.replace(hour=next_hour, minute=15, second=0, microsecond=0)
        else:
            # We're in an odd hour, next turn is at :15 of next even hour
