...
from Systems.Functions.utils import get_local_ip, get_service_port, SERVICE_COMPARE, release_port
...
class CompareCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.port = get_service_port(SERVICE_COMPARE)  # Use port manager
        self.public_url = None
        try:
            self.query_instance = create_v3_query_instance(logger=self.logger)
    ...
    def start_web_server(self):
        ...
        try:
            ...
            self.logger.info(f"Started web server on port {self.port} serving from {web_dir}")
            if ngrok:
                try:
                    authtoken = os.environ.get("NGROK_AUTHTOKEN")
                    if authtoken:
                        ngrok.set_auth_token(authtoken)
                        self.logger.info("ngrok authtoken found and set.")

                    for tunnel in ngrok.get_tunnels():
                        if tunnel.proto == 'http' and tunnel.config['addr'].endswith(f":{self.port}"):
                            ngrok.disconnect(tunnel.public_url)
                            self.logger.info(f"Disconnected existing ngrok tunnel: {tunnel.public_url}")

                    public_tunnel = ngrok.connect(self.port, "http")
                    self.public_url = public_tunnel.public_url
                    self.logger.info(f"ngrok tunnel opened at: {self.public_url}")
                except Exception as e:
                    self.logger.error(f"Could not start ngrok tunnel: {e}")
            else:
                self.logger.warning("pyngrok is not installed. Public URL will not be available.")
        except OSError as e:
            if e.winerror == 10048: # Address already in use
                 self.logger.warning(f"Port {self.port} is already in use. Assuming server is running.")
            else:
                raise

    def _nation_fields_minimal(self) -> str:
        """Returns a string of the minimal nation fields required for the compare command."""
        return """
            id
            nation_name
            alliance_position
    ...
    def cog_unload(self):
        ...
        if ngrok and self.public_url:
            self.logger.info(f"Closing ngrok tunnel {self.public_url}...")
            ngrok.disconnect(self.public_url)
            self.public_url = None
            self.logger.info("ngrok tunnel closed.")
        # Release the allocated port
        release_port(SERVICE_COMPARE)
    ...
    async def _resolve_targets(self, text: str) -> List[Tuple[Optional[int], Optional[str]]]:
        ...
        for part in parts:
            ...
            r_id, r_name = await self._resolve_alliance_id_from_api(name or part)
    ...
    async def _get_nations_batched_minimal_recursive(self, alliance_ids: List[str], page: int = 1, prev_nations: Optional[Dict[str, List[Dict[str, Any]]]] = None) -> Dict[str, List[Dict[str, Any]]]:
        """Recursively fetch nations for multiple alliances with pagination using minimal fields."""
        if not alliance_ids:
            return {}

        nations_by_alliance = prev_nations or {aid: [] for aid in alliance_ids}
        
        queries = {}
        for aid in alliance_ids:
            queries[f"a{aid}_p{page}"] = f"""
                nations(alliance_id: {aid}, first: 500, page: {page}) {{
                    paginatorInfo {{ hasMorePages }}
                    data {{
                        {self._nation_fields_minimal()}
                    }}
                }}
            """
        
        batch_results = await self.query_instance.execute_batch(queries)
        
        more_pages_to_fetch = []
        for aid in alliance_ids:
            alias = f"a{aid}_p{page}"
            result = batch_results.get(alias, {})
            if result and isinstance(result, dict):
                nations_data = result.get('data')
                if isinstance(nations_data, list):
                    nations_by_alliance[aid].extend([self.query_instance._normalize_nation(n) for n in nations_data])
                
                paginatorInfo = result.get('paginatorInfo')
                if paginatorInfo and paginatorInfo.get('hasMorePages', False):
                    more_pages_to_fetch.append(aid)
        
        if more_pages_to_fetch:
            return await self._get_nations_batched_minimal_recursive(more_pages_to_fetch, page + 1, nations_by_alliance)
        
        return nations_by_alliance

    async def _get_nations_batched_minimal(self, ids: List[Union[int, str]], side_label: str) -> Dict[int, List[Dict[str, Any]]]:
        """Fetch nations for multiple alliance IDs using a single batched API call with minimal fields."""
        if not ids:
            return {}

        str_ids = [str(aid) for aid in ids]
        
        try:
            self.logger.info(f"Processing all alliances for {side_label} in a single minimal batch: {str_ids}")
            all_batched_nations = await self._get_nations_batched_minimal_recursive(str_ids)

            out = {int(aid): nations for aid, nations in all_batched_nations.items()}
            return out
        except Exception as e:
            self.logger.error(f"Error in _get_nations_batched_minimal: {e}")
            out = {}
            for aid in ids:
                int_aid = int(aid)
                try:
                    ...
                    self.logger.error(f"Fallback fetch failed for alliance {aid}: {ex}")
                    out[int_aid] = []
            return out
    ...
    async def _get_nations_batched_recursive(self, alliance_ids: List[str], page: int = 1, prev_nations: Optional[Dict[str, List[Dict[str, Any]]]] = None) -> Dict[str, List[Dict[str, Any]]]:
        """Recursively fetch nations for multiple alliances with pagination."""
        if not alliance_ids:
            return {}

        nations_by_alliance = prev_nations or {aid: [] for aid in alliance_ids}
        
        # Construct batch query for the current page
        queries = {}
        for aid in alliance_ids:
            queries[f"a{aid}_p{page}"] = f"""
                nations(alliance_id: {aid}, first: 500, page: {page}) {{
                    paginatorInfo {{ hasMorePages }}
                    data {{
                        {self.query_instance._nation_fields()}
                    }}
                }}
            """
        ...
        for aid in alliance_ids:
            ...
            if result and isinstance(result, dict):
                ...
                if result.get('paginatorInfo', {}).get('hasMorePages', False):
                    more_pages_to_fetch.append(aid)
        
        # If there are more pages, recurse
        if more_pages_to_fetch:
            return await self._get_nations_batched_recursive(more_pages_to_fetch, page + 1, nations_by_alliance)
        
        return nations_by_alliance

    async def _get_nations_batched(self, ids: List[Union[int, str]], side_label: str) -> Dict[int, List[Dict[str, Any]]]:
        """Fetch nations for multiple alliance IDs using a single batched API call."""
        if not ids:
            return {}
        ...
        try:
            self.logger.info(f"Processing all alliances for {side_label} in a single batch: {str_ids}")
            all_batched_nations = await self._get_nations_batched_recursive(str_ids)

            # Convert keys to integers for the final output
            out = {int(aid): nations for aid, nations in all_batched_nations.items()}
            return out
        except Exception as e:
            self.logger.error(f"Error in _get_nations_batched: {e}")
            # Fallback to individual fetches if batching fails
            out = {}
            for aid in ids:
                int_aid = int(aid)
                try:
    ...
    async def compare(self, interaction: discord.Interaction, home: str, away: str):
        try:
            ...
            parsed_home: List[Tuple[Optional[int], Optional[str]]] = await self._resolve_targets(home)
            ...
            home_batched: Dict[int, List[Dict[str, Any]]] = await self._get_nations_batched_minimal(cast(List[Union[int, str]], home_ids), side_label='home')
            ...
            away_batched: Dict[int, List[Dict[str, Any]]] = await self._get_nations_batched_minimal(cast(List[Union[int, str]], away_ids), side_label='away')
            ...
            for aid, name in valid_home:
                nations = home_batched.get(aid, [])
                if nations:
                    stats = await self._compute_stats(nations)
                    daily_military = await self._compute_full_mill(nations)
                    stats['daily_military'] = daily_military
                    city_counts = {}
                    for n in self._get_active_nations(nations):
                        num_cities = n.get('num_cities', 0)
                        city_counts[num_cities] = city_counts.get(num_cities, 0) + 1
                    stats['city_counts'] = city_counts
                    home_individual_stats.append({'name': name, 'stats': stats})

            away_individual_stats = []
            for aid, name in valid_away:
                nations = away_batched.get(aid, [])
                if nations:
                    stats = await self._compute_stats(nations)
                    daily_military = await self._compute_full_mill(nations)
                    stats['daily_military'] = daily_military
                    ...
                    for n in self._get_active_nations(nations):
                        ...
                        city_counts[num_cities] = city_counts.get(num_cities, 0) + 1
                    stats['city_counts'] = city_counts
                    away_individual_stats.append({'name': name, 'stats': stats})
            
            all_individual_stats = [home_individual_stats, away_individual_stats]
            ...
            html_content = create_interactive_comparison_page(home_individual_stats, away_individual_stats)
            ...
            html_path = os.path.join(web_dir, "comparison_graph.html")
            ...
            if self.public_url:
                url = f"{self.public_url}/comparison_graph.html"
                embed.add_field(name="Interactive View", value=f"[Click Here for a **VERY** detailed view]({url})", inline=False)
            else:
                local_ip = get_local_ip()
                localhost_url = f"http://localhost:{self.port}/comparison_graph.html"
                embed.add_field(name="Interactive View", value=f"[Click Here for a **VERY** detailed view]({localhost_url})", inline=False)
                if local_ip and local_ip != '127.0.0.1':
                    lan_url = f"http://{local_ip}:{self.port}/comparison_graph.html"
                    embed.add_field(name="LAN View", value=f"`{lan_url}`", inline=False)
