...
class DestroyCog(commands.Cog):
    ...
    async def destroy_target_autocomplete(self, interaction: discord.Interaction, current: str) -> List[app_commands.Choice[str]]:
        ...
        try:
            ...
            # Get nations from all configured alliances for autocomplete
            all_nations = []
            try:
                # Get all alliance IDs from config or use a fallback
                # For now, we'll try to get from any cached alliance data
                # This could be improved by storing configured alliances
                alliance_ids = []
                if self.home_alliance_id:
                    alliance_ids.append(str(self.home_alliance_id))
    ...
    async def destroy(
        ...
        try:
            ...
            view = self.create_optimal_attackers_view(interaction, target_nation, attackers_list, show_warchest=False)
    ...
    def create_optimal_attackers_view(self, interaction: discord.Interaction, target_nation: Dict[str, Any], optimal_attackers: Dict[str, Any], show_warchest: bool = False) -> Optional[Any]:
