...
from Systems.Pets.PetGames.dungeon_crawl import (
    ...
    CHEST_EVENT_MAP, TRAP_EFFECTS, SHRINE_EFFECTS, CHEST_TYPES
)
...
async def start_battle(dungeon_id: str, request: Request, body: StartBattleRequest = None):
    ...
    try:
        ...
        active_battles[battle_id] = {
            ...
            'monster_charge': 1.0,
            'party_charges': {p['user_id']: 1.0 + p['charge'] for p in party_data},
...
async def process_battle_turn(battle: Dict) -> Dict:
    ...
    try:
        from Systems.Pets.Logic.battle_skills import (
            apply_skill, tick_battle_effects, is_stunned, consume_stun,
    ...
    for member in party:
        ...
        if action == 'skill' and _skills_available:
            ...
            for slot_idx, skill_id in enumerate(equipped):
                if can_use_skill(state, slot_idx):
                    monster_state = {
                        'element': str(monster.get('element', 'basic')).lower(),
                        'active_effects': [],
                        'max_hp': monster['max_health'],
                    }
                    result = apply_skill(skill_id, state, monster_state,
                    ...
                    if result['ok']:
                        ...
                        if hp_delta_user != 0:
                            pet['health'] = max(0, min(pet['max_health'],
                                                       pet['health'] + hp_delta_user))
                        turn_log.append({
        ...
        if action == 'attack':
            ...
            new_charge = min(charge_limit, current_charge + 1.0)
            party_charges[uid] = new_charge
            turn_log.append({'actor': pet['name'], 'action': 'defend',
                             'message': f"{pet['name']} defends! (Charge ×{new_charge:.0f})"})

        elif action == 'charge':
            new_charge = min(charge_limit, current_charge + 2.0)
            party_charges[uid] = new_charge
    ...
    if monster_action == 'attack':
        ...
        effective_target_def = int(target_pet['defense'] * t_def_mult)

        battle_result = DamageCalculator.calculate_battle_action(
            attacker_attack=monster['attack'],
        ...
        monster_charge = min(8.0, monster_charge + 2.0)
    ...
    # Build per-user skill cooldown map for the frontend
    skill_cooldowns = {}
    for uid, state in skill_states.items():
        cds = state.get('skill_cooldowns', {})
        skill_cooldowns[uid] = cds.get(0, 0)  # slot 0 cooldown
...
async def trigger_trap(dungeon_id: str, request: Request):
    ...
    try:
        ...
        if not trap:
            # Fallback only if data missing
            trap = random.choice(TRAP_EFFECTS)
        
        # Apply trap effect to party
        dungeon.apply_trap_effect(trap)
...
async def activate_shrine(dungeon_id: str, request: Request):
    ...
    try:
        ...
        if not shrine:
            # Fallback only if data missing
            shrine = random.choice(SHRINE_EFFECTS)
        
        # Apply shrine effect to party
        dungeon.apply_shrine_effect(shrine)
